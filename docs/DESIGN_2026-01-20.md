...
### Staff Role Contract (Source of Truth)
- Staff authority is derived from explicit auth/token metadata only.
- Canonical parser: `web/src/auth/staffRole.ts`.
- Precedence (highest to lowest):
  1. `claims.admin === true` or `claims.roles` includes `admin`
  2. `claims.staff === true` or `claims.roles` includes `staff`
  3. Fallback record role (`admin`/`staff`/`member`) for merged Firestore member records only
  4. Default `member`
- Staff-gated web paths now consume the same parser contract:
  - `web/src/App.tsx` (shell/nav staff gate)
  - `web/src/views/ReservationsView.tsx` (staff workflow claim checks)
  - `web/src/views/StaffView.tsx` (staff console authority + member role normalization)
...
4) Keep role-contract docs/tests in sync when auth claims change.
...
8) Reservations TODOs:
   ...
   - Digital Studio Glaze Board module (chip grid with photos, cone + surface filters, favorites, admin CRUD, mobile-first layout).
9) Tooling TODOs:
   - Remove any remaining concurrently references now that dev uses scripts/dev.mjs.
