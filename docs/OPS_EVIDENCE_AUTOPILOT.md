...
## Script
- `node ./scripts/ps1-run.mjs scripts/run-ops-evidence-autopilot.ps1`
...
3. Notification reliability drills (`node ./scripts/ps1-run.mjs scripts/run-notification-drills.ps1`) when token + uid are supplied
...
node ./scripts/ps1-run.mjs scripts/run-ops-evidence-autopilot.ps1 -StudioDrills both
```
