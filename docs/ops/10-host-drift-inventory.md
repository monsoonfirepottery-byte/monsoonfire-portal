...
- output classifies `matched`, `drift`, `missing_remote`, `unreadable_remote`, `generated_remote`, and `untracked_remote` candidates.
...
- 2 installed unit files are generated by the tracked `scripts/install-studiobrain-portal-bridge.sh` installer rather than represented as static config files:
  - `/etc/systemd/system/studio-brain-control-tower-proxy.service`
  - `/etc/systemd/system/studio-brain-namecheap-tunnel.service`

Safe next step: keep those two service definitions visible in portal-bridge install reviews, or split them into templated config files in a later PR if static drift comparison becomes more valuable than install-time interpolation. Do not overwrite or remove either service from this report alone.
