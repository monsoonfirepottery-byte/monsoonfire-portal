...
- The wrapper defaults to `/home/wuff/monsoonfire-portal` and supports override environment variables for repo root, Node path, profile, jobs, wiki mode, repo depth, strict mode, and load threshold.
...
- The install script enables and restarts the timers, but does not explicitly start either idle-worker service. Because the timers are persistent, first-time installation should still be treated as an approval-gated host change.
