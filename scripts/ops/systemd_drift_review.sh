...
LOCAL_MODE=0
STRICT=0
GENERATED_REMOTE_SOURCE="scripts/install-studiobrain-portal-bridge.sh"
GENERATED_REMOTE_PATHS=(
  "${REMOTE_SYSTEMD_ROOT}/studio-brain-control-tower-proxy.service"
  "${REMOTE_SYSTEMD_ROOT}/studio-brain-namecheap-tunnel.service"
)
...
declare -A generated_remote_paths=()

for generated_path in "${GENERATED_REMOTE_PATHS[@]}"; do
  generated_remote_paths["${generated_path}"]=1
done
...
if [ -z "${remote_paths}" ]; then
  ...
  while IFS= read -r remote_path; do
    ...
    if [ -n "${expected_remote_paths[${remote_path}]+x}" ]; then
      continue
    fi
    if [ -n "${generated_remote_paths[${remote_path}]+x}" ]; then
      generated_count=$((generated_count + 1))
      printf 'generated_remote\t%s\t%s\n' "${GENERATED_REMOTE_SOURCE}" "${remote_path}"
    else
      untracked_count=$((untracked_count + 1))
      printf 'untracked_remote\t%s\n' "${remote_path}"
    fi
  done <<< "${remote_paths}"
  if [ "${untracked_count}" -eq 0 ] && [ "${generated_count}" -eq 0 ]; then
    echo "none"
  fi
fi
...
printf 'generated_remote_candidates: %s\n' "${generated_count}"
...
generated_remote: host file is generated by a tracked install script rather than represented as a static config file.
