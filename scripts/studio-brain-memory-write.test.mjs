...
test("rememberWithStudioBrain routes single saves through capture and writes a local audit row", async () => {
  ...
  try {
    ...
    assert.equal(calls[0].body.sourceClass, "human");
    ...
    assert.equal(Array.isArray(calls[0].body.evidence), true);
    assert.equal(calls[0].body.evidence[0].sourceClass, "human");
...
test("rememberWithStudioBrain batches multiple saves through import and updates continuity state", async () => {
  ...
  try {
    ...
    assert.equal(calls[0].body.items[0].sourceClass, "runtime-artifact");
    assert.equal(calls[0].body.items[1].sourceClass, "runtime-artifact");
    ...
    assert.equal(Array.isArray(calls[0].body.items[0].evidence), true);
    assert.equal(calls[0].body.items[0].evidence[0].sourceClass, "runtime-artifact");
