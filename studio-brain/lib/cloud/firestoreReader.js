...
exports.readFirestoreModel = readFirestoreModel;
...
async function safeReadCollection(params) {
    ...
    try {
        ...
        return {
            name,
            docs: snapshot.docs.map((doc) => ({ data: () => doc.data() })),
            size: snapshot.size,
...
async function readFirestoreModel(projectId, scanLimitOverride) {
    const startedAt = Date.now();
    ...
    for (const doc of firingsRead.docs) {
        ...
        if (["queued", "scheduled", "loaded", "firing", "cooling", "ready"].includes(status))
            firingsScheduled += 1;
    }
    let reportsOpen = 0;
    ...
    for (const doc of reportsRead.docs) {
        ...
        if (["open", "triaged", "actioned"].includes(status))
            reportsOpen += 1;
        if (severity === "high")
    ...
    for (const doc of agentRequestsRead.docs) {
        ...
        if (["pending", "needs_review", "awaiting_payment"].includes(status))
            agentRequestsPending += 1;
    }
    let pendingOrders = 0;
    for (const doc of ordersRead.docs) {
        const status = String(doc.data().status ?? "").toLowerCase();
        if (!["paid", "cancelled", "fulfilled", "completed"].includes(status))
            pendingOrders += 1;
    }
