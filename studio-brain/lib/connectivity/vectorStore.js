...
function isFingerprintDuplicateError(error) {
...
function toScoredLane(row, lane) {
    return {
        lane,
...
async function hasPgExtension(extname) {
...
async function detectEmbeddingStorageMode() {
    const pool = (0, postgres_1.getPgPool)();
    const result = await pool.query(`
    ...
    if (typeName.includes("vector"))
        return "vector";
    if (typeName.includes("double precision[]") || typeName.includes("real[]"))
        return "float8-array";
    return "none";
}
async function createVectorStore(logger = noOpLogger()) {
    ...
    if (supportsVector && embeddingStorageMode === "float8-array") {
        logger.warn("vector_store_embedding_column_legacy_array_mode", {
            message: "swarm_memory.embedding is double precision[]; running in compatibility mode with runtime casts to vector.",
        });
    }
    ...
    const upsertMemory = async (item) => {
        ...
        await (0, retry_1.withRetry)("vector_store_upsert", async () => {
            ...
            const embeddingValue = embeddingStorageMode === "vector"
                ? hasEmbedding
                ...
                : hasEmbedding
                    ? sanitizedEmbedding
                    : null;
            const embeddingCast = embeddingStorageMode === "vector"
                ? "$16::vector"
          ...
          INSERT INTO swarm_memory (
            ...
            $1, $2, $3, $4, $5, $6, $7::jsonb, $8::timestamptz, $9, $10, $11, $12, $13, $14, $15${embeddingWriteEnabled ? `, ${embeddingCast}` : ""}, now(), now(), now()
          )
          ON CONFLICT (memory_id) DO UPDATE SET
            ...
            embedding_version = EXCLUDED.embedding_version${embeddingWriteEnabled ? ", embedding = EXCLUDED.embedding" : ""},
    ...
    const searchMemory = async (params) => {
        ...
        if (useSemanticLane) {
            ...
            const distanceExpr = embeddingStorageMode === "float8-array"
                ? `(embedding::vector <=> ${vectorRef}::vector)`
                : `embedding <=> ${vectorRef}::vector`;
            const semanticResult = await (0, retry_1.withRetry)("vector_store_search_semantic", async () => withQueryTimeout(queryTimeoutMs, "search-semantic", () => pool.query(`
                SELECT
                  memory_id,
                  tenant_id,
                  agent_id,
                 ...
                 LIMIT ${limitRef}
                `, scoped.values)), logger, { attempts: 1 });
            laneRows.push(...semanticResult.rows.map((row) => toScoredLane(row, "semantic")));
        }
        if (useLexicalLane) {
            const scoped = createScopedFilter({
                tenantId: params.tenantId ?? null,
            ...
            const limitRef = scoped.push(candidateLimit);
            const lexicalResult = queryText.length < 2
                ? await (0, retry_1.withRetry)("vector_store_search_lexical_short_query", async () => withQueryTimeout(queryTimeoutMs, "search-lexical-short", () => pool.query(`
                      SELECT
                        memory_id,
                        tenant_id,
                        agent_id,
                       ...
                       LIMIT ${limitRef}
                      `, scoped.values)), logger, { attempts: 1 })
                : await (0, retry_1.withRetry)("vector_store_search_lexical", async () => {
                    ...
                    return withQueryTimeout(queryTimeoutMs, "search-lexical", () => pool.query(`
                      SELECT
                        memory_id,
                        tenant_id,
                        agent_id,
            ...
            laneRows.push(...lexicalResult.rows.map((row) => toScoredLane(row, "lexical")));
        }
        if (useSessionLane) {
            const scoped = createScopedFilter({
                tenantId: params.tenantId ?? null,
            ...
            const limitRef = scoped.push(candidateLimit);
            const sessionResult = await withQueryTimeout(queryTimeoutMs, "search-session", () => pool.query(`
            SELECT
              memory_id,
              tenant_id,
              agent_id,
             ...
             LIMIT ${limitRef}
            `, scoped.values));
            laneRows.push(...sessionResult.rows.map((row) => toScoredLane(row, "session")));
        }
        const laneBuckets = {
            semantic: [],
            lexical: [],
        ...
        const rrfK = 60;
        for (const [lane, rows] of Object.entries(laneBuckets)) {
            for (const [index, row] of rows.entries()) {
                ...
                if (lane === "semantic")
                    existing.semantic = row.laneScore;
                if (lane === "lexical")
                    existing.lexical = row.laneScore;
                if (lane === "session")
                    existing.sessionLane = row.laneScore;
            }
        }
