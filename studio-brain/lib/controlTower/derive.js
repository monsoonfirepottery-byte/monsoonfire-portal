...
exports.resolveEscalatedRoomIds = resolveEscalatedRoomIds;
exports.buildControlTowerEvents = buildControlTowerEvents;
exports.deriveControlTowerState = deriveControlTowerState;
exports.deriveRoomDetail = deriveRoomDetail;
...
function actionTargetForService(serviceId) {
    return { type: "service", serviceId };
}
function actionTargetForRoom(roomId) {
    return { type: "room", roomId };
}
function actionTargetForSession(sessionName) {
    return { type: "session", sessionName };
}
function toSeverityFromHealth(health) {
    if (health === "error")
        return "critical";
    if (health === "waiting")
        return "warning";
...
function normalizeChannel(value) {
    ...
    if (normalized.includes("service"))
        return "service";
    if (normalized.includes("ops") || normalized.includes("operator"))
        return "ops";
    return "unknown";
}
function classifyChannel(tool, ...hints) {
...
function eventTypeFromInput(input) {
    ...
    if (sourceAction.includes("proposal") || sourceAction.includes("approval"))
        return "approval.requested";
    if (sourceAction.includes("memory"))
        return "memory.promoted";
    if (input.serviceId || input.kind === "service")
        return "health.changed";
    if (input.kind === "session")
        return "run.status";
    if (input.kind === "alert" || input.kind === "overseer" || input.severity === "critical")
        return "incident.raised";
    if (input.roomId || input.kind === "room" || input.kind === "ack" || input.kind === "operator")
        return "task.updated";
    return "task.updated";
}
...
function summarizeRoom(room) {
    ...
    if (!room.sessions.length)
        return "No active sessions in this room.";
    return `${room.sessions.length} lane${room.sessions.length === 1 ? "" : "s"} in ${room.label}.`;
}
function buildRoomNextActions(room, isEscalated, nowMs) {
...
function toRoomSummary(room, escalatedRoomIds, nowMs) {
    ...
    const status = room.mood === "blocked"
        ? "blocked"
        : room.mood === "quiet"
            ? "quiet"
    ...
    return {
        ...
        status,
        objective: (0, collect_1.clipText)(primarySession?.objective || primarySession?.summary || summarizeRoom(room), 180),
        lastActivityAt,
        ageMinutes,
        isEscalated,
...
function toServiceCard(service, events) {
    ...
    const impact = service.status === "error"
...
function toEventFromAck(entry, index) {
    const at = coerceString(entry.recordedAt);
    if (!at)
        return null;
    const runId = coerceString(entry.runId) || "latest";
    const note = (0, collect_1.clipText)(entry.note || "Overseer acknowledgement recorded.", 220);
    return createEvent({
        id: `ack:${runId}:${index}`,
        at,
        kind: "ack",
...
function toEventFromAlert(state, index) {
...
function eventKindFromAction(action) {
    if (action.includes("service"))
        return "service";
    if (action.includes("ack"))
        return "ack";
    if (action.includes("session") || action.includes("room"))
        return "room";
    return "operator";
}
function severityFromAudit(event) {
    const action = String(event.action || "").toLowerCase();
    if (action.includes("restart") || action.includes("stop") || action.includes("blocked"))
        return "warning";
    return event.approvalState === "required" ? "warning" : "info";
}
function titleFromAudit(event) {
    const action = String(event.action || "");
    if (action === "studio_ops.control_tower.room_pinned")
        return "Room escalated";
    if (action === "studio_ops.control_tower.room_unpinned")
        return "Room de-escalated";
    if (action === "studio_ops.control_tower.session_spawned")
        return "Room session created";
    if (action === "studio_ops.control_tower.session_instruction_sent")
        return "Instruction sent";
    if (action === "studio_ops.control_tower.service_action")
        return "Service action run";
    if (action === "studio_ops.control_tower.overseer_ack")
        return "Overseer ack recorded";
    return (0, collect_1.clipText)(action.replaceAll(".", " "), 72) || "Operator action";
}
function toEventFromAudit(event) {
    ...
    return createEvent({
        ...
        summary: (0, collect_1.clipText)(event.rationale || JSON.stringify(metadata), 220),
        actor: event.actorId,
        roomId: coerceString(metadata.roomId) || null,
        serviceId: coerceString(metadata.serviceId) || null,
        actionLabel: coerceString(metadata.roomId)
            ? "Inspect room"
...
function toEventFromOverseerAction(action, computedAt, index) {
    return createEvent({
        id: `overseer:${action.id || index}`,
        at: computedAt,
        kind: "overseer",
        severity: action.priority === "p0" ? "critical" : action.priority === "p1" ? "warning" : "info",
        title: (0, collect_1.clipText)(action.title || "Overseer recommendation", 96),
        summary: (0, collect_1.clipText)(action.summary || action.rationale || "", 220),
        actor: "overseer",
...
function resolveEscalatedRoomIds(events) {
    ...
    for (const event of roomEvents) {
        ...
        if (!roomId)
            continue;
        if (event.action === "studio_ops.control_tower.room_pinned")
            output.add(roomId);
        else
            output.delete(roomId);
    }
    return output;
}
function buildControlTowerEvents(raw, audits) {
    const events = [];
...
function buildNeedsAttention(raw, rooms, services, events) {
    const items = [];
...
function buildOverseerNextActions(raw) {
    ...
    return actions.slice(0, 3).map((entry, index) => ({
        ...
        title: (0, collect_1.clipText)(entry.title || "Review overseer recommendation", 96),
        why: (0, collect_1.clipText)(entry.rationale || entry.summary || "", 180),
        ageMinutes: minutesSince(raw.ops.overseer?.computedAt),
        actionLabel: "Review recommendation",
        target: { type: "ops", action: "overseer" },
    }));
}
function buildGoodNextMoves(raw, rooms) {
    const actions = [];
...
function buildMissionBoard(rows) {
    return rows
        .map((room) => ({
        id: `board:${room.id}`,
        owner: room.tool || room.name,
        task: (0, collect_1.clipText)(room.objective || room.summary || room.name, 120),
        state: room.isEscalated ? `escalated:${room.status}` : room.status,
        blocker: room.status === "blocked"
            ? (0, collect_1.clipText)(room.summary || "Blocked lane.", 140)
            : room.isEscalated
                ? "Operator escalation is still open."
                : "",
        next: (0, collect_1.clipText)(room.nextActions[0]?.title || "Inspect room", 120),
        ...
        const rightState = right.state.startsWith("escalated") ? 0 : right.state === "blocked" ? 1 : right.state === "waiting" ? 2 : 3;
...
function buildChannelSummaries(raw) {
    return raw.agents
        .map((session) => ({
        ...
        objective: (0, collect_1.clipText)(session.objective || session.summary || session.sessionName, 140),
        blocker: session.status === "error" ? (0, collect_1.clipText)(session.summary || "Lane is blocked.", 120) : "",
...
function buildFallbackMemoryBrief(raw, rooms, needsAttention, actions, events) {
    const goal = (0, collect_1.clipText)(rooms[0]?.objective || "Rehydrate operator continuity and identify the next safe move.", 160);
    const blockers = needsAttention.slice(0, 3).map((item) => (0, collect_1.clipText)(item.title, 120));
    const recentDecisions = events
        .filter((event) => event.kind === "operator" || event.kind === "ack" || event.kind === "service")
        .slice(0, 3)
        .map((event) => (0, collect_1.clipText)(`${event.title}: ${event.summary}`, 140));
    const recommendedNextActions = actions.slice(0, 3).map((action) => (0, collect_1.clipText)(action.title, 120));
    ...
    return {
        ...
        sourcePath: null,
        layers: {
            coreBlocks: [goal, (0, collect_1.clipText)(raw.ops.summary || "Control Tower continuity is pending.", 140)],
            workingMemory: actions.slice(0, 2).map((action) => (0, collect_1.clipText)(action.why, 140)),
...
function deriveRoomDetail(raw, state, roomId, attach) {
