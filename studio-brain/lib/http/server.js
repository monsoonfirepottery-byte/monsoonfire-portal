...
function parseSignedOpsSessionPrincipal(value) {
    ...
    const portalRole = portalRoleRaw === "admin" || portalRoleRaw === "staff" || portalRoleRaw === "member" ? portalRoleRaw : "member";
    const opsRoles = toStringList(payload.opsRoles, 32).filter((entry) => [
        "owner",
        "member_ops",
        "support_ops",
        "kiln_lead",
    ...
    const opsCapabilities = toStringList(payload.opsCapabilities, 96).filter(Boolean);
...
function verifySignedOpsSessionToken(token, secret) {
    ...
    try {
        ...
        if (!principal || principal.uid !== decoded.sub)
            return null;
        return principal;
    }
    catch {
    ...
    const server = node_http_1.default.createServer(async (req, res) => {
        ...
        try {
            ...
            if (opsPortal.enabled && opsService && opsDisplayPageMatch) {
                ...
                const sessionToken = auth.ok && auth.principal && opsSessionSecret
