...
function parseSource(metadata, fallback) {
...
function sanitizeSourceList(values, maxItems = 40) {
...
function sanitizeStatusList(values, maxItems = 8) {
    if (!Array.isArray(values))
        return [];
    const out = new Set();
...
function parseStringList(value, maxItems = 64) {
...
function normalizeEntityHints(raw, maxItems = 32) {
    if (!Array.isArray(raw))
        return [];
    const seen = new Set();
    const out = [];
...
function normalizePatternHints(raw, maxItems = 32) {
    if (!Array.isArray(raw))
        return [];
    const seen = new Set();
    const out = [];
...
function normalizeSignalEdgeKeys(values, memoryId, maxItems = 192) {
    if (!Array.isArray(values))
        return [];
    return Array.from(new Set(values
        .map((entry) => {
...
function normalizeSignalEntityKeys(values, maxItems = 256) {
    if (!Array.isArray(values))
        return [];
    return Array.from(new Set(values
        .map((entry) => {
...
function normalizeSignalPatternKeys(values, maxItems = 320) {
    if (!Array.isArray(values))
        return [];
    return Array.from(new Set(values
        .map((entry) => {
...
function parseStatus(value) {
    const raw = String(value ?? "").trim().toLowerCase();
    if (raw === "accepted" || raw === "quarantined" || raw === "archived")
        return raw;
    return "proposed";
}
function parseMemoryType(value) {
    const raw = String(value ?? "").trim().toLowerCase();
    if (raw === "working" || raw === "semantic" || raw === "procedural")
        return raw;
    return "episodic";
}
...
function parseRedactionState(value) {
    const raw = String(value ?? "").trim().toLowerCase();
    if (raw === "none" ||
        raw === "redacted" ||
...
function latticeTextSql(topLevelKey, nestedKey) {
    return `COALESCE(NULLIF(LOWER(metadata->>'${topLevelKey}'), ''), NULLIF(LOWER(metadata->'memoryLattice'->>'${nestedKey}'), ''))`;
}
function latticeNumericSql(topLevelKey, nestedKey, fallback = "0") {
    return `
    CASE
...
function mapCountRows(rows, key) {
    return rows
        .map((row) => ({
        value: String(row[key] ?? "").trim().toLowerCase(),
...
function mapTransitionRow(row) {
    return {
        ...
        fromTruthStatus: row.from_truth_status === null ? null : String(row.from_truth_status),
        toTruthStatus: String(row.to_truth_status),
        fromFreshnessStatus: row.from_freshness_status === null ? null : String(row.from_freshness_status),
        toFreshnessStatus: String(row.to_freshness_status),
        fromOperationalStatus: row.from_operational_status === null ? null : String(row.from_operational_status),
        toOperationalStatus: String(row.to_operational_status),
        evidenceIds: parseStringList(row.evidence_ids, 32),
        metadata: asRecord(row.metadata),
    };
}
function parseReviewCaseType(value) {
    const raw = String(value ?? "").trim().toLowerCase();
...
function parseReviewCaseStatus(value) {
    const raw = String(value ?? "").trim().toLowerCase();
...
function parseReviewCaseAction(value) {
    const raw = String(value ?? "").trim().toLowerCase();
    if (raw === "verify_now" ||
        raw === "accept_winner" ||
...
function parseVerifierKind(value) {
    const raw = String(value ?? "").trim().toLowerCase();
    if (raw === "repo-head" ||
        raw === "runtime-check" ||
...
function parseVerificationTrigger(value) {
    const raw = String(value ?? "").trim().toLowerCase();
...
function parseVerificationResultStatus(value) {
    const raw = String(value ?? "").trim().toLowerCase();
...
function mapVerificationRunRow(row) {
    return {
        id: String(row.id ?? ""),
        tenantId: row.tenant_id === null ? null : String(row.tenant_id),
...
function createPostgresMemoryStoreAdapter(params) {
    ...
    const attachRecordDetails = async (rows) => {
        ...
        const evidenceByMemoryId = new Map();
        ...
        const transitionsByMemoryId = new Map();
    ...
    const persistLatticeProjection = async (client, input) => {
        ...
        const shadowMcpRisk = metadata.shadowMcpRisk === true || lattice.shadowMcpRisk === true || mcpGovernance.shadowRisk === true;
    ...
    const replaceEvidence = async (client, input) => {
    ...
    const upsertTransitionEvents = async (client, input) => {
        if (!Array.isArray(input.transitionEvents) || input.transitionEvents.length === 0)
            return;
    ...
    return {
        async upsert(input) {
          ...
          `, [
                    `mie_${(0, node_crypto_1.randomUUID)()}`,
                    input.tenantId ?? null,
                    input.source,
                    input.status,
                    input.id,
        ...
        async search(input) {
            ...
            const matchedByCounts = enriched.reduce((acc, row) => {
          ...
          `, [
                    `mre_${(0, node_crypto_1.randomUUID)()}`,
                    input.tenantId ?? null,
                    input.agentId ?? null,
                    input.runId ?? null,
                    input.query,
      ...
      `;
        ...
        async getByIds(input) {
            ...
            if (!ids.length)
                return [];
            const values = [ids];
            const predicates = ["memory_id = ANY($1::text[])"];
            if (input.tenantId !== undefined) {
                values.push(input.tenantId);
      ...
      `;
        ...
        `, [input.tenantId ?? null]);
            ...
            return {
                ...
                lattice: {
                    ...
                    byCategory: mapCountRows(byCategory.rows, "category").map((row) => ({
                        category: row.value,
                        count: row.count,
                    })),
                    byTruthStatus: mapCountRows(byTruthStatus.rows, "status").map((row) => ({
                        status: row.value,
                        count: row.count,
                    })),
                    byFreshnessStatus: mapCountRows(byFreshnessStatus.rows, "status").map((row) => ({
                        status: row.value,
                        count: row.count,
                    })),
                    byOperationalStatus: mapCountRows(byOperationalStatus.rows, "status").map((row) => ({
                        status: row.value,
                        count: row.count,
                    })),
                    byReviewAction: mapCountRows(byReviewAction.rows, "action").map((row) => ({
                        action: row.value,
                        count: row.count,
                    })),
                    backlog,
        ...
        `, [
                ...
                JSON.stringify(Array.from(new Set((input.recommendedActions ?? []).map((entry) => parseReviewCaseAction(entry)))).slice(0, 16)),
                input.owner ?? null,
        ...
        async indexSignals(input) {
            ...
            const patterns = Array.isArray(input.patterns)
                ? input.patterns
                    ...
                    .slice(0, 128)
                : [];
            if (edges.length === 0 && entities.length === 0 && patterns.length === 0)
                return;
            try {
                await pool.query(`
          ...
          `, [tenantScope, memoryId]);
            ...
            `, [tenantScope, memoryId, pattern.patternType, pattern.patternKey, pattern.patternValue, pattern.confidence]);
                ...
                if (message.includes('relation "memory_relation_edge" does not exist') ||
        ...
        async hasSignalIndex(input) {
            ...
            const indexed = (edgeKeys.length === 0 || edgeMatches >= edgeKeys.length) &&
        ...
        async updateLoopState(input) {
          ...
          SELECT current_state, last_state_confidence
           ...
           LIMIT 1
          `, [tenantScope, loopKey]);
                const previousState = previous.rowCount && previous.rows[0] ? normalizeLoopState(previous.rows[0].current_state) : null;
                const previousConfidence = previous.rowCount && previous.rows[0] ? clamp01(previous.rows[0].last_state_confidence, 0.5) : null;
                await pool.query(`
          ...
          `, [
                    ...
                    JSON.stringify(input.metadata && typeof input.metadata === "object" && !Array.isArray(input.metadata)
                        ? input.metadata
                        : {}),
                ]);
                try {
            ...
            `, [
                        `mlte_${(0, node_crypto_1.randomUUID)()}`,
                        tenantScope,
                        loopKey,
                        previousState,
                        state,
        ...
        async searchLoopState(input) {
            ...
            const values = [tenantScope];
            const predicates = ["tenant_scope = $1"];
        ...
        async recordLoopFeedback(input) {
          ...
          `, [
                    `mlfe_${(0, node_crypto_1.randomUUID)()}`,
                    tenantScope,
                    loopKey,
                    action,
                    actorId,
        ...
        async searchLoopFeedbackStats(input) {
            ...
            const values = [tenantScope];
            const predicates = ["tenant_scope = $1", `occurred_at >= now() - make_interval(days => $2::int)`];
