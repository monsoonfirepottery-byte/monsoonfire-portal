...
const SECRET_ASSIGNMENT_PATTERN = /\b(api[-_ ]?key|access[-_ ]?token|refresh[-_ ]?token|id[-_ ]?token|bearer token|authorization|password|session cookie)\b\s*[:=]\s*([^\s,;]+)/gi;
...
const MEMORY_LATTICE_REVIEW_DAYS = {
    observation: 14,
    fact: 90,
    decision: 180,
    guardrail: 180,
...
function shouldRedactMetadataKey(key) {
    ...
    if (SAFE_METADATA_POINTER_KEY_PATTERN.test(normalized))
        return false;
    if (SAFE_CONTROL_METADATA_KEY_PATTERN.test(normalized))
        return false;
...
function normalizeMemoryTruthStatus(value) {
    const raw = String(value ?? "").trim().toLowerCase();
    if (raw === "observed" ||
        raw === "inferred" ||
        raw === "proposed" ||
...
function normalizeMemoryFreshnessStatus(value) {
    const raw = String(value ?? "").trim().toLowerCase();
...
function normalizeMemoryOperationalStatus(value) {
    const raw = String(value ?? "").trim().toLowerCase();
    if (raw === "active" ||
        raw === "cooling" ||
        raw === "quarantined" ||
...
function normalizeMemoryAuthorityClass(value) {
    const raw = String(value ?? "").trim().toLowerCase();
    if (raw === "a0-live" ||
        raw === "a1-repo" ||
        raw === "a2-policy" ||
...
function normalizeMemorySourceClass(value) {
    const raw = String(value ?? "").trim().toLowerCase();
    if (raw === "live-check" ||
        raw === "repo-file" ||
        raw === "policy" ||
...
function normalizeMemoryRedactionState(value) {
    const raw = String(value ?? "").trim().toLowerCase();
    if (raw === "none" ||
        raw === "redacted" ||
...
function normalizeMemoryConflictSeverity(value) {
    const raw = String(value ?? "").trim().toLowerCase();
...
function normalizeMemoryLoopState(value) {
    const raw = String(value ?? "").trim().toLowerCase();
...
function authorityStrength(authorityClass) {
    if (authorityClass === "a0-live")
        return 1;
    if (authorityClass === "a1-repo")
        return 0.96;
    if (authorityClass === "a2-policy")
        return 0.92;
    if (authorityClass === "a3-telemetry")
        return 0.76;
    if (authorityClass === "a4-derived")
        return 0.54;
    return 0.34;
}
...
function hasImplicitEvidence(metadata) {
    return Boolean(normalizeText(metadata.sourceArtifactPath) ||
...
function deriveMemorySourceClass(input) {
    const explicit = normalizeMemorySourceClass(input.metadata.sourceClass ?? normalizeMetadata(input.metadata.memoryLattice).sourceClass);
    if (explicit)
        return explicit;
    const source = normalizeSource(input.source);
    const toolClass = normalizeSource(String(input.metadata.toolClass ?? ""));
    if (metadataFlag(input.metadata, "authoritative") ||
...
function scanSensitiveContent(content) {
    ...
    const replaceIfMatched = (pattern, replacement, reason) => {
        let matched = false;
...
function buildMemoryEvidenceId(parts) {
    return `evidence:${(0, node_crypto_1.createHash)("sha1").update(parts.map((part) => String(part ?? "")).join("|")).digest("hex").slice(0, 24)}`;
}
function normalizeCaptureEvidence(input) {
    ...
    if (sourceArtifactPath || corpusRecordId || sourceUri) {
        derived.push({
            ...
            sourceClass: sourceArtifactPath || corpusRecordId ? "repo-file" : sourceUri.startsWith("http") ? "external-doc" : input.sourceClass,
    ...
    const provided = Array.isArray(input.provided)
        ? input.provided.map((entry, index) => {
            ...
            const sourceClass = normalizeMemorySourceClass(entry.sourceClass) ||
...
function buildMemoryTransitionEvents(input) {
    ...
    if (!changed)
        return [];
    const transitionId = `transition:${(0, node_crypto_1.createHash)("sha1")
        .update([
        input.memoryId,
...
function reviewWindowDaysForCategory(category) {
    return MEMORY_LATTICE_REVIEW_DAYS[category] ?? 45;
}
function contradictionCountFromMetadata(metadata) {
    ...
    const quarantinedConflict = normalizeText(normalizeMetadata(metadata.quarantinedByConsolidation).reason) === "conflicting-loop-state" ? 1 : 0;
    return Math.max(explicitCount, listCount, loopConflict, quarantinedConflict);
}
function conflictingMemoryIdsFromMetadata(metadata) {
    return Array.from(new Set(readStringValues([
        ...
        .filter((value) => value.startsWith("mem_") || value.startsWith("dream-connection:") || value.startsWith("conflict:")))).slice(0, 24);
}
function conflictKindsFromMetadata(metadata) {
    return Array.from(new Set(readStringValues([
...
function deriveMemoryConflictSeverity(metadata, contradictionCount) {
    const explicit = normalizeMemoryConflictSeverity(metadata.conflictSeverity ?? normalizeMetadata(metadata.memoryLattice).conflictSeverity);
    if (explicit)
        return explicit;
    if (contradictionCount <= 0)
        return "none";
    const kinds = conflictKindsFromMetadata(metadata);
    if (metadata.conflictingLoopState === true ||
...
function mergeConflictSeverity(left, right) {
    return conflictSeverityRank(right) > conflictSeverityRank(left) ? right : left;
}
function isContradictionRelationHit(hit) {
    return Boolean(hit?.relationTypes.some((entry) => normalizeText(entry).trim().toLowerCase() === "contradicts"));
}
function shouldSurfaceQuarantinedConflictRow(row, hit, useMode, anchorMs = Date.now()) {
    if (row.status !== "quarantined")
        return true;
    if (!isContradictionRelationHit(hit))
        return false;
    if (useMode !== "planning" && useMode !== "debugging" && useMode !== "exploratory")
        return false;
    const lattice = withMemoryLatticeRecord(row, anchorMs).lattice;
    return Boolean(lattice &&
...
function applyConflictShadowToSearchRows(rows, companionRows, anchorMs = Date.now()) {
    if (rows.length === 0 || companionRows.length === 0)
        return rows;
    ...
    for (const companion of companionRows) {
        ...
        if (!lattice)
            continue;
        if (lattice.category !== "conflict-record" &&
            lattice.truthStatus !== "contradicted" &&
            lattice.contradictionCount <= 0 &&
            lattice.conflictSeverity === "none") {
            continue;
        }
        const implicatedIds = new Set(lattice.conflictingMemoryIds
            .map((value) => normalizeText(value))
            .filter((value) => seedIds.has(value)));
        if (implicatedIds.size === 0)
            continue;
        for (const seedId of implicatedIds) {
            ...
            current.reviewPriority = Math.max(current.reviewPriority, lattice.conflictSeverity === "hard" ? 0.96 : lattice.reviewAction === "resolve-conflict" ? 0.9 : 0.82);
            shadowById.set(seedId, current);
        }
    }
    if (shadowById.size === 0)
        return rows;
    return rows.map((row) => {
        ...
        const contradictionCount = Math.max(lattice.contradictionCount, mergedSeverity === "hard" ? 1 : lattice.contradictionCount > 0 ? lattice.contradictionCount : 0);
        ...
        const nextLattice = {
            ...
            conflictingMemoryIds: Array.from(new Set([...lattice.conflictingMemoryIds, ...shadow.conflictingMemoryIds])).slice(0, 24),
...
function applyConflictShadowToRecords(rows, anchorMs = Date.now()) {
    if (rows.length === 0)
        return rows;
    const shadowed = applyConflictShadowToSearchRows(rows.map((row) => toSearchResultFromRecord(row, undefined, anchorMs)), rows, anchorMs);
    return shadowed.map(({ score, scoreBreakdown, matchedBy, ...row }) => row);
}
function deriveMemoryAuthorityClass(input) {
    const explicit = normalizeMemoryAuthorityClass(input.metadata.authorityClass ?? normalizeMetadata(input.metadata.memoryLattice).authorityClass);
    if (explicit)
        return explicit;
    if (input.sourceClass === "live-check")
        return "a0-live";
    if (input.sourceClass === "repo-file")
        return "a1-repo";
    if (input.sourceClass === "policy")
        return "a2-policy";
    if (input.sourceClass === "telemetry" ||
        input.sourceClass === "runtime-artifact" ||
        input.sourceClass === "mcp-tool" ||
        input.sourceClass === "external-doc") {
        return "a3-telemetry";
    }
    if (input.sourceClass === "derived")
        return "a4-derived";
    const source = normalizeSource(input.source);
    if (metadataFlag(input.metadata, "authoritative") ||
        metadataFlag(input.metadata, "liveCheck") ||
...
function deriveMemoryCategory(input) {
    const explicit = normalizeMemoryCategory(input.metadata.memoryCategory ??
    ...
    const hasHint = (...needles) => hints.some((value) => needles.some((needle) => value.includes(needle)));
    if (contradictionCountFromMetadata(input.metadata) > 0 && (hasHint("conflict") || content.includes("conflict:"))) {
        return "conflict-record";
    }
    if (hasHint("guardrail", "policy", "safety", "constraint") ||
    ...
    if (hasHint("known-bug", "bug", "failure-pattern", "incident", "footgun") ||
    ...
    if (input.memoryType === "procedural" ||
    ...
    if (hasHint("derived", "insight", "synthesis") ||
...
function deriveMemoryScope(metadata) {
    ...
    if (projectLane)
        return `lane:${projectLane}`;
    return null;
}
function deriveFreshnessTimestamps(input) {
    const explicitLastVerifiedAt = normalizeText(input.metadata.lastVerifiedAt ??
        input.metadata.verifiedAt ??
        normalizeMetadata(input.metadata.memoryLattice).lastVerifiedAt);
    const explicitNextReviewAt = normalizeText(input.metadata.nextReviewAt ?? normalizeMetadata(input.metadata.memoryLattice).nextReviewAt);
    const explicitFreshnessExpiresAt = normalizeText(input.metadata.freshnessExpiresAt ?? normalizeMetadata(input.metadata.memoryLattice).freshnessExpiresAt);
...
function deriveMemoryTruthStatus(input) {
    const explicit = normalizeMemoryTruthStatus(input.metadata.truthStatus ?? normalizeMetadata(input.metadata.memoryLattice).truthStatus);
    if (explicit)
        return explicit;
    if (input.status === "quarantined" || contradictionCountFromMetadata(input.metadata) > 0)
        return "contradicted";
    if (input.category === "hypothesis")
        return input.status === "accepted" ? "inferred" : "proposed";
    if (input.category === "observation")
        return input.status === "accepted" ? "observed" : "proposed";
    if (input.status === "proposed") {
        return input.authorityClass === "a4-derived" || input.authorityClass === "a5-inferred" ? "proposed" : "inferred";
    }
    if (input.status === "accepted") {
        if (input.authorityClass === "a0-live" ||
...
function deriveMemoryFreshnessStatus(input) {
    const explicit = normalizeMemoryFreshnessStatus(input.metadata.freshnessStatus ?? normalizeMetadata(input.metadata.memoryLattice).freshnessStatus);
    if (explicit)
        return explicit;
    ...
    if (remainingMs <= reviewWindowMs * 0.33)
        return "aging";
    return "fresh";
}
function deriveMemoryOperationalStatus(input) {
    const explicit = normalizeMemoryOperationalStatus(input.metadata.operationalStatus ?? normalizeMetadata(input.metadata.memoryLattice).operationalStatus);
    if (explicit)
        return explicit;
    if (normalizeText(input.metadata.retiredAt))
        return "retired";
    if (input.status === "archived")
        return "archived";
    if (input.status === "quarantined" || input.truthStatus === "contradicted")
        return "quarantined";
    if (input.category === "legacy-lore" ||
        normalizeText(input.metadata.deprecatedAt) ||
        normalizeText(input.metadata.supersededBy) ||
...
function deriveFolkloreRisk(input) {
    ...
    let risk = 0.05;
    const source = normalizeSource(input.source);
    if (input.authorityClass === "a4-derived")
        risk += 0.36;
    if (input.authorityClass === "a5-inferred")
        risk += 0.26;
    if (source.includes("compaction"))
        risk += 0.18;
    if (input.truthStatus === "proposed" || input.truthStatus === "inferred")
        risk += 0.16;
    if (input.truthStatus === "contradicted")
        risk += 0.24;
    if (input.freshnessStatus === "revalidation-required" || input.freshnessStatus === "stale")
        risk += 0.2;
    if (input.category === "legacy-lore" || input.category === "derived-insight")
        risk += 0.1;
...
function deriveMemoryReviewPlan(input) {
    const explicitAction = normalizeMemoryReviewAction(input.metadata.reviewAction ?? normalizeMetadata(input.metadata.memoryLattice).reviewAction);
    const explicitPriority = Number(input.metadata.reviewPriority ?? normalizeMetadata(input.metadata.memoryLattice).reviewPriority);
    const explicitReasons = readStringValues(input.metadata.reviewReasons ?? normalizeMetadata(input.metadata.memoryLattice).reviewReasons, 8)
        .map((value) => normalizeSource(value))
        .filter(Boolean)
        .slice(0, 6);
    if (explicitAction) {
    ...
    const redactionState = normalizeMemoryRedactionState(input.metadata.redactionState ?? normalizeMetadata(input.metadata.memoryLattice).redactionState);
    const secretExposure = normalizeMetadata(input.metadata.secretExposure);
    const shadowMcpRisk = input.metadata.shadowMcpRisk === true || normalizeMetadata(input.metadata.mcpGovernance).shadowRisk === true;
    const highImpactCategory = input.category === "fact" ||
    ...
    if (secretExposure.detected === true || redactionState === "requires-review" || redactionState === "quarantined") {
        ...
        if (input.operationalStatus === "quarantined")
            reasons.add("quarantined");
        if (input.truthStatus === "contradicted" || input.contradictionCount > 0)
            reasons.add("contradicted");
    }
    ...
    else if (input.folkloreRisk >= 0.65 &&
    ...
    if (input.folkloreRisk >= 0.65)
        reasons.add("folklore-risk");
    if (input.category === "workaround")
        reasons.add("temporary");
    return {
        reviewAction,
...
function buildMemoryLatticeSnapshot(input) {
    ...
    const evidenceStrength = clamp01(authorityStrength(authorityClass) * 0.45 + input.sourceConfidence * 0.25 + (hasEvidence ? 0.3 : 0.05), input.sourceConfidence);
    const redactionState = normalizeMemoryRedactionState(input.metadata.redactionState ?? normalizeMetadata(input.metadata.memoryLattice).redactionState);
    const secretExposureMetadata = normalizeMetadata(input.metadata.secretExposure);
    const secretExposure = secretExposureMetadata.detected === true ||
        secretExposureMetadata.quarantined === true ||
        secretExposureMetadata.canonicalPromotionBlocked === true;
    const shadowMcpRisk = input.metadata.shadowMcpRisk === true ||
...
function withMemoryLatticeRecord(row, anchorMs = Date.now()) {
    if (row.lattice)
        return row;
    const metadata = normalizeMetadata(row.metadata);
    return {
...
function withMemoryLatticeSearchResult(row, anchorMs = Date.now()) {
    if (row.lattice)
        return row;
    return {
        ...row,
...
function allowsMemoryForUseMode(lattice, useMode) {
    if (lattice.operationalStatus === "retired")
        return false;
    if (useMode === "safety-critical") {
        if (lattice.operationalStatus !== "active")
            return false;
        if (lattice.truthStatus !== "verified" && lattice.truthStatus !== "trusted")
            return false;
        if (lattice.freshnessStatus !== "fresh")
            return false;
        if (lattice.category === "hypothesis" ||
    ...
    if (useMode === "human-facing") {
        if (lattice.operationalStatus === "quarantined")
            return false;
        if (lattice.authorityClass === "a5-inferred")
            return false;
        if (lattice.category === "conflict-record")
            return false;
        return true;
    ...
    if (useMode === "planning") {
        if (lattice.operationalStatus === "archived")
            return false;
        if (lattice.operationalStatus !== "quarantined")
            return true;
...
function useModePenalty(lattice, useMode) {
    let penalty = 0;
    if (lattice.freshnessStatus === "aging")
        penalty += useMode === "safety-critical" ? 0.12 : 0.04;
    if (lattice.freshnessStatus === "revalidation-required")
        penalty += useMode === "safety-critical" ? 0.28 : 0.12;
    if (lattice.freshnessStatus === "stale")
        penalty += useMode === "safety-critical" ? 0.4 : 0.18;
    if (lattice.truthStatus === "proposed" || lattice.truthStatus === "inferred")
        penalty += useMode === "planning" ? 0.04 : 0.12;
    if (lattice.truthStatus === "contradicted")
        penalty += 0.36;
    if (lattice.conflictSeverity === "hard")
        penalty += 0.08;
    if (lattice.conflictSeverity === "soft")
        penalty += 0.03;
    if (lattice.operationalStatus === "cooling")
        penalty += 0.03;
    if (lattice.operationalStatus === "deprecated")
        penalty += 0.22;
    if (lattice.operationalStatus === "archived")
        penalty += 0.3;
    if (lattice.operationalStatus === "quarantined")
...
function isContestedLattice(lattice) {
    return (lattice.truthStatus === "contradicted" ||
        lattice.operationalStatus === "quarantined" ||
        lattice.conflictSeverity === "hard" ||
        lattice.contradictionCount > 0);
}
function retrievalAnchorAtMs(row) {
    const anchor = row.lattice?.lastVerifiedAt || row.occurredAt || row.createdAt;
    const parsed = Date.parse(anchor ?? "");
    return Number.isFinite(parsed) ? parsed : null;
}
function matchesRetrievalPolicy(row, policy, anchorMs = Date.now()) {
...
function applyRetrievalPolicyToSearchRows(rows, policy, anchorMs = Date.now()) {
    return rows.filter((row) => matchesRetrievalPolicy(row, policy, anchorMs));
}
function shouldOverfetchForPolicy(policy) {
    return Boolean(policy.fillToValidLimit ||
...
function applyUseModeToSearchRows(rows, useMode, anchorMs = Date.now()) {
    return rows
        .map((row) => {
        ...
        if (lattice.freshnessStatus !== "fresh")
            matchedBy.push(`freshness:${lattice.freshnessStatus}`);
        if (lattice.operationalStatus === "quarantined" || lattice.truthStatus === "contradicted")
            matchedBy.push("contested");
        if (lattice.conflictSeverity !== "none")
            matchedBy.push(`conflict:${lattice.conflictSeverity}`);
        if (lattice.folkloreRisk >= 0.55)
            matchedBy.push("folklore-risk");
        if (lattice.reviewAction !== "none")
            matchedBy.push(`review:${lattice.reviewAction}`);
        return {
            ...withLattice,
...
function applyUseModeToRecords(rows, useMode, anchorMs = Date.now()) {
    return rows
        .map((row) => withMemoryLatticeRecord(row, anchorMs))
        .filter((row) => row.lattice ? allowsMemoryForUseMode(row.lattice, useMode) : true);
}
function finalizeSearchRows(rows, policy, anchorMs = Date.now()) {
    return diversifyRankedRows(applyRetrievalPolicyToSearchRows(applyUseModeToSearchRows(filterExpiredSearchResults(rows, anchorMs), policy.useMode, anchorMs), policy, anchorMs), policy.limit);
}
...
function summarizeContextItems(items, maxChars = 480) {
    ...
    for (const [index, row] of items.entries()) {
        ...
        if (hints.some((entry) => entry.includes("priority:urgent")) || metadataFlag(metadata, "urgentLike"))
            flags.push("URGENT");
        if (row.lattice?.freshnessStatus === "stale")
            flags.push("STALE");
        if (row.lattice?.freshnessStatus === "revalidation-required")
            flags.push("VERIFY");
        if (row.lattice?.truthStatus === "contradicted" || row.lattice?.operationalStatus === "quarantined")
            flags.push("CONTESTED");
        if (row.lattice?.reviewAction && row.lattice.reviewAction !== "none")
            flags.push("REVIEW");
...
function summarizeConflictBlockedContext(items, useMode, maxChars = 480) {
    ...
    if (hardConflictItems.length === 0)
        return "";
    const scopes = Array.from(new Set(hardConflictItems
...
function isCoreContextOnlySearchResult(row) {
    if (row.memoryLayer === "core")
        return true;
    if (row.source === "startup-context")
        return true;
    if (row.matchedBy.includes("core-block"))
        return true;
    return row.tags.includes("core-block");
}
function deriveBlockedByHardConflictDiagnostic(items, useMode, selectedRows) {
    if ((useMode !== "operational" && useMode !== "safety-critical") || items.length === 0) {
        return undefined;
    }
    const hardConflictItems = items.filter((row) => row.lattice?.conflictSeverity === "hard");
    ...
    if (selectedBlockingRows.length > 0)
        return undefined;
    const conflictRecord = hardConflictItems.find((row) => row.lattice?.category === "conflict-record");
    const scope = hardConflictItems
        .map((row) => normalizeText(row.lattice?.scope))
        .find(Boolean)
        ?? null;
    const conflictingMemoryIds = Array.from(new Set(hardConflictItems.flatMap((row) => row.lattice?.conflictingMemoryIds ?? []))).slice(0, 24);
    return {
        blocked: true,
        useMode,
        scope,
...
function inferProjectLaneFromQuery(query) {
    const text = query.toLowerCase();
    if (/\bmonsoonfire[- ]portal\b|\bportal\.monsoonfire\.com\b/.test(text))
...
function scoreRecentRowsForQuery(rows, query, options = {}) {
...
function createMemoryService(options) {
    ...
    const buildSearchFallbackCacheKey = (params) => stableStringify({
        ...
        maxStalenessHours: typeof params.maxStalenessHours === "number" ? Math.max(0, Math.trunc(params.maxStalenessHours)) : null,
    ...
    const memoryEventTimestampMs = (occurredAt, createdAt) => Number.isFinite(Date.parse(occurredAt || createdAt || "")) ? Date.parse(occurredAt || createdAt || "") : Date.now();
    const exactLoopStateConflictReason = (input) => {
        if (input.incomingState === input.existingState)
            return null;
        ...
        if (input.incomingState === "superseded" || input.existingState === "superseded")
            return null;
        if (input.incomingState === "reopened" && input.existingState === "resolved" && incomingMs > existingMs)
            return null;
    ...
    const findExactCaptureConflicts = async (input) => {
        const matches = [];
        const scope = deriveMemoryScope(input.metadata);
        const incomingState = normalizeMemoryLoopState(input.metadata.loopState ?? input.metadata.currentState ?? input.metadata.state);
        ...
        if (explicitConflictIds.length > 0) {
            ...
            for (const row of explicitRows) {
                if (!row || row.id === input.incomingId)
                    continue;
                matches.push({
                    memoryId: row.id,
        ...
        const eligibleCategory = input.category === "decision" ||
            input.category === "guardrail" ||
            input.category === "fact" ||
        ...
        for (const row of recentRows) {
            ...
            if (deriveMemoryScope(rowMetadata) !== scope)
                continue;
    ...
    const memoryReviewCaseGetRequestSchema = zod_1.z.object({
        tenantId: zod_1.z.string().trim().min(1).max(128).nullable().optional(),
        id: zod_1.z.string().trim().min(1).max(160),
    });
    const isOpenReviewCaseStatus = (status) => status === "open" || status === "in-progress";
    const deriveReviewCaseTypeForRow = (row) => {
        const metadata = normalizeMetadata(row.metadata);
    ...
    const recommendedActionsForCaseType = (caseType) => {
        switch (caseType) {
    ...
    const verifierKindForCase = (caseType, metadata, scope) => {
        ...
        if (caseType === "promote-guidance")
            return "support-outcome";
        if (caseType === "resolve-conflict" && scope?.startsWith("subject:"))
            return "repo-head";
        if (caseType === "revalidate" && scope?.startsWith("subject:"))
            return "startup-instruction";
        if (caseType === "retire")
            return "operator-attested";
        return "repo-head";
    };
    const buildReviewCaseId = (input) => `review:${(0, node_crypto_1.createHash)("sha1")
        .update([
        input.tenantId ?? "none",
        input.caseType,
        input.scope ?? "",
        ...
        .digest("hex")
        .slice(0, 24)}`;
    const mergeEvidenceRows = (existing, additions = []) => {
        const merged = new Map();
    ...
    const reindexRecordSignals = async (row) => {
        if (!options.store.indexSignals)
            return;
        try {
            await options.store.indexSignals(deriveSignalIndex({
    ...
    const upsertReviewCase = async (input) => {
        if (!options.store.upsertReviewCase || !options.store.listReviewCases)
            return null;
        const linkedMemoryIds = Array.from(new Set(input.linkedMemoryIds.map((value) => normalizeText(value)).filter(Boolean))).slice(0, 64);
        if (linkedMemoryIds.length === 0 && !input.scope)
            return null;
        const existing = (await options.store.listReviewCases({
            tenantId: input.tenantId,
    ...
    const buildVerificationEvidence = (input) => {
        ...
        const sourceClass = input.verifierKind === "runtime-check"
            ? "live-check"
            : input.verifierKind === "repo-head" || input.verifierKind === "startup-instruction"
                ...
                : input.verifierKind === "support-policy"
                    ...
                    : input.verifierKind === "support-outcome"
                        ...
                        : "human";
        return {
            evidenceId: `evidence:verify:${(0, node_crypto_1.createHash)("sha1")
    ...
    const executeVerificationRun = async (input) => {
        ...
        const runId = `verify:${(0, node_crypto_1.createHash)("sha1")
        ...
        else if ((targetWithLattice.lattice.truthStatus === "verified" || targetWithLattice.lattice.truthStatus === "trusted")
        ...
        if (resultStatus === "passed") {
            ...
            const verifiedTarget = await updateMemoryLifecycle({
                ...
                truthStatus: targetWithLattice.lattice.truthStatus === "trusted" ? "trusted" : "verified",
                freshnessStatus: "fresh",
                operationalStatus: input.reviewCase.caseType === "retire" ? "retired" : targetWithLattice.lattice.operationalStatus === "retired" ? "retired" : "active",
        ...
        const downgradedTarget = await updateMemoryLifecycle({
            ...
            operationalStatus: input.reviewCase.caseType === "resolve-conflict" || resultStatus === "failed"
                ? "quarantined"
                : targetWithLattice.lattice.operationalStatus,
            truthStatus: input.reviewCase.caseType === "resolve-conflict" && resultStatus === "failed"
                ? "contradicted"
    ...
    const ensureReviewCaseForRows = async (input) => {
        ...
        if (rowsWithLattice.length === 0)
            return null;
        const primary = rowsWithLattice[0];
        const hardConflict = rowsWithLattice.some((row) => row.lattice?.conflictSeverity === "hard");
        const caseType = hardConflict ? "resolve-conflict" : deriveReviewCaseTypeForRow(primary) ?? (primary.lattice?.reviewAction === "retire" ? "retire" : "revalidate");
        const linkedMemoryIds = Array.from(new Set(rowsWithLattice.flatMap((row) => [
            row.id,
    ...
    const capture = async (raw, captureOptions) => {
        ...
        const ttlAwareMetadata = derivedLayer === "working"
            ...
            : metadataWithLayer;
        const lineageAwareMetadata = normalizeMetadata(derivedLayer === "canonical"
        ...
        const exactConflictMatches = captureOptions?.skipConflictDetection === true ||
        ...
        const conflictReasons = Array.from(new Set([
        ...
        const conflictingMemoryIds = Array.from(new Set([
            ...conflictingMemoryIdsFromMetadata(lineageAwareMetadata),
            ...exactConflictMatches.map((match) => match.memoryId),
        ])).slice(0, 24);
        const contradictionCount = Math.max(contradictionCountFromMetadata(lineageAwareMetadata), conflictReasons.length > 0 ? conflictReasons.length : conflictingMemoryIds.length > 0 ? 1 : 0);
        ...
        const conflictAugmentedMetadata = {
            ...
            conflictSeverity: exactConflictMatches.some((match) => match.severity === "hard")
                ? "hard"
                : contradictionCount > 0
                    ? explicitConflictSeverity || "soft"
                    : explicitConflictSeverity || "none",
            conflictingMemoryIds,
            relatedMemoryIds: Array.from(new Set([
                ...readStringValues(lineageAwareMetadata.relatedMemoryIds, 32),
                ...conflictingMemoryIds,
            ])).slice(0, 32),
            conflictScope: deriveMemoryScope(lineageAwareMetadata),
            conflictDetectedAt: contradictionCount > contradictionCountFromMetadata(lineageAwareMetadata) || exactConflictMatches.length > 0
                ? new Date().toISOString()
                : normalizeText(lineageAwareMetadata.conflictDetectedAt) || undefined,
            conflictingLoopState: lineageAwareMetadata.conflictingLoopState === true || exactConflictMatches.some((match) => match.kind === "exact-loop-state"),
        };
        ...
        const rawMcpGovernance = normalizeMetadata(conflictAugmentedMetadata.mcpGovernance);
        const mcpShadowRisk = sourceClass === "mcp-tool" &&
            String(rawMcpGovernance.approvalState ?? "").trim().toLowerCase() !== "approved";
        const mcpGovernance = sourceClass === "mcp-tool" || Object.keys(rawMcpGovernance).length > 0
            ? {
                ...rawMcpGovernance,
        ...
        const transitionEvents = buildMemoryTransitionEvents({
            ...
            reason: sensitiveContent.detected
                ? `secret-exposure:${sensitiveContent.reasons.join(",")}`
                : normalizeText(conflictAugmentedMetadata.transitionReason) || parsed.clientRequestId || normalizedSource,
            at: createdAt,
        });
        const metadata = redactSensitiveMetadata({
            ...trustMetadata,
        ...
        if (captureOptions?.skipConflictDetection !== true &&
            ...
            const statePairs = Array.from(new Set(exactConflictMatches.map((match) => [match.incomingState || "unknown", match.existingState || "unknown"].join(" vs ")))).slice(0, 6);
            const scopeLabel = conflictScope.replace(/^subject:/, "").replace(/^thread:/, "").replace(/^loop:/, "");
            const conflictId = `conflict:${(0, node_crypto_1.createHash)("sha1")
            ...
            await capture({
                ...
                runId: stored.runId,
                content: `Conflict: ${scopeLabel || conflictScope} has conflicting memory claims. `
    ...
    const search = async (raw) => {
        ...
        const finalizeAndMaybeBackfill = async (rowsInput, conflictCompanions = []) => {
            const shadowedScoped = applyConflictShadowToSearchRows(applyRequestedScope(rowsInput), [...rowsInput, ...conflictCompanions], Date.now());
            const scoped = shadowedScoped.sort((left, right) => right.score - left.score || right.createdAt.localeCompare(left.createdAt));
            ...
            const fallbackRows = filterExpiredMemoryRecords(await options.store.recent({
                tenantId,
                agentId: parsed.agentId,
                runId: parsed.runId,
                sourceAllowlist: allowSources,
            ...
            const mergedById = new Map(scoped.map((row) => [row.id, row]));
            ...
            finalized = finalizeSearchRows(applyConflictShadowToSearchRows(mergedRows, [...mergedRows, ...conflictCompanions], Date.now()), retrievalPolicy);
            if ((parsed.useMode === "operational" || parsed.useMode === "safety-critical") && finalized.length > 0) {
                await ensureReviewCaseForRows({
                    tenantId,
                    rows: finalized.filter((row) => row.lattice?.reviewAction && row.lattice.reviewAction !== "none"),
    ...
    const recent = async (raw) => {
        ...
        const tenantId = normalizeTenant(parsed.tenantId);
        return applyUseModeToRecords(applyConflictShadowToRecords(filterExpiredMemoryRecords(await options.store.recent({
            tenantId,
    ...
    const listReviewCases = async (raw) => {
        if (!options.store.listReviewCases)
            return [];
        let parsed;
        try {
            parsed = contracts_1.memoryReviewCaseListRequestSchema.parse(raw ?? {});
        }
        catch (error) {
            throw normalizeError(error);
        }
    ...
    const getReviewCase = async (raw) => {
        if (!options.store.getReviewCaseById)
            return null;
        let parsed;
        try {
    ...
    const listVerificationRuns = async (raw) => {
        if (!options.store.listVerificationRuns)
            return [];
        let parsed;
        try {
            parsed = zod_1.z
                .object({
                tenantId: zod_1.z.string().trim().min(1).max(128).nullable().optional(),
                caseId: zod_1.z.string().trim().min(1).max(160).nullable().optional(),
                targetMemoryId: zod_1.z.string().trim().min(1).max(128).nullable().optional(),
    ...
    const reviewCaseAction = async (raw) => {
        ...
        try {
            parsed = zod_1.z
                ...
                .merge(contracts_1.memoryReviewCaseActionRequestSchema)
                .parse(raw ?? {});
        }
        catch (error) {
            throw normalizeError(error);
        ...
        const resolveCase = async (status, resolution, overrides = {}) => upsertReviewCase({
            tenantId: reviewCase.tenantId,
            caseType: reviewCase.caseType,
            scope: reviewCase.scope,
            primaryMemoryId: reviewCase.primaryMemoryId,
        ...
        switch (parsed.action) {
            case "verify_now": {
                ...
                if (verification.target)
                    updatedMemoryIds.add(verification.target.id);
                const nextStatus = verification.run?.resultStatus === "passed" && (reviewCase.caseType === "revalidate" || reviewCase.caseType === "retire")
                    ? "resolved"
                    : "in-progress";
                return {
                    reviewCase: await resolveCase(nextStatus, verification.run?.resultSummary ?? "Verification requested.", { lastVerificationRunId: verification.run?.id ?? reviewCase.lastVerificationRunId }),
                    verificationRun,
            ...
            case "retire_memory": {
                ...
                verificationRun = (await executeVerificationRun({
                    reviewCase,
                    targetMemoryId: selectedMemoryId,
                    actorId,
                    trigger: "review-action",
    ...
    const context = async (raw) => {
        ...
        const rankedConflictCompanions = [];
        ...
        ranked = applyRetrievalPolicyToSearchRows(applyUseModeToSearchRows(ranked.sort((left, right) => right.score - left.score || right.createdAt.localeCompare(left.createdAt)), parsed.useMode, temporalAnchorMs), retrievalPolicy, temporalAnchorMs);
        ...
        const finalSelected = applyRetrievalPolicyToSearchRows(applyUseModeToSearchRows(applyConflictShadowToSearchRows(selected, [...selected, ...rankedConflictCompanions], temporalAnchorMs), parsed.useMode, temporalAnchorMs), retrievalPolicy, temporalAnchorMs).slice(0, parsed.maxItems);
        ...
        if (parsed.useMode === "operational" || parsed.useMode === "safety-critical") {
            ...
            const reviewCase = reviewRows.length > 0
                ? await ensureReviewCaseForRows({
                    tenantId: tenantResolution.tenantId,
                    rows: reviewRows,
                    useMode: parsed.useMode,
        ...
        if (query && finalSelected.length > 0) {
            writeContextFallbackCache(contextFallbackCacheKey, finalSelected.slice(0, Math.max(parsed.maxItems * 3, parsed.maxItems)), retrievalModeUsed);
        }
        return {
            summary: (blockedByHardConflict
    ...
    const consolidate = async (raw = {}) => {
        ...
        const isNonRetryableAssociationScoutFailure = (error) => {
        ...
        const scoutAssociationBundle = async (bundle) => {
            if (!associationScout)
                return null;
            if (associationScoutDisabledAfterFailure)
                return null;
