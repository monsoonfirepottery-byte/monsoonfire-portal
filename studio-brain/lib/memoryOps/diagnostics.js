...
exports.createMemoryOpsRunner = createMemoryOpsRunner;
exports.collectMemoryHttpSnapshot = collectMemoryHttpSnapshot;
exports.collectPostgresDbaSnapshot = collectPostgresDbaSnapshot;
exports.collectSystemdServiceProbes = collectSystemdServiceProbes;
exports.collectDockerServiceProbes = collectDockerServiceProbes;
exports.collectProcessProbes = collectProcessProbes;
const node_child_process_1 = require("node:child_process");
const node_path_1 = require("node:path");
const postgres_1 = require("../db/postgres");
const collect_1 = require("../controlTower/collect");
function createMemoryOpsRunner() {
...
async function fetchJson(baseUrl, path, headers, timeoutMs) {
...
function memoryStatsSummary(payload) {
    const stats = payload && typeof payload === "object" ? payload.stats : null;
    if (!stats)
        return null;
    const reviewBacklog = stats.reviewBacklog;
    const conflictBacklog = stats.conflictBacklog;
...
async function collectMemoryHttpSnapshot(input) {
    ...
    try {
        ...
        pressure = payload && typeof payload === "object" ? (payload.pressure ?? null) : null;
    }
    catch {
        pressure = null;
    }
...
async function collectPostgresDbaSnapshot() {
    const checkedAt = new Date().toISOString();
    const startedAt = Date.now();
    try {
        const pool = (0, postgres_1.getPgPool)();
        const [settings, activity, longQueries, tableStats, indexStats, databaseStats, extensions,] = await Promise.all([
...
function collectSystemdServiceProbes(input) {
    const checkedAt = new Date().toISOString();
...
function collectDockerServiceProbes(input) {
    const checkedAt = new Date().toISOString();
    const repoRoot = (0, collect_1.resolveControlTowerRepoRoot)(input.repoRoot);
    const composeFile = input.composeFile || (0, node_path_1.resolve)(repoRoot, "studio-brain", "docker-compose.yml");
    const result = input.runner("docker", ["compose", "-f", composeFile, "ps", "--services", "--status", "running"], {
        cwd: (0, node_path_1.resolve)(repoRoot, "studio-brain"),
    });
    const expected = [
    ...
    return expected.map((service) => {
        ...
        return {
            ...
            label: service.label,
            kind: "docker",
            health: isRunning ? "healthy" : optional ? "unknown" : "critical",
...
function parsePgrep(stdout) {
    return stdout
        ...
        .filter((entry) => Boolean(entry && entry.pid > 1));
}
function collectProcessProbes(input) {
    const checkedAt = new Date().toISOString();
    ...
    const probes = [];
    const buildProbe = (id, label, rowsForProbe, healthyMax) => {
        probes.push({
            id,
            label,
            kind: "process",
            ...
            summary: rowsForProbe.length <= healthyMax
