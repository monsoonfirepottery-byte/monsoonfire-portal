...
exports.classifyPostgresDbaFindings = classifyPostgresDbaFindings;
exports.classifyMemoryFindings = classifyMemoryFindings;
exports.classifyServiceFindings = classifyServiceFindings;
exports.deriveMemoryOpsActions = deriveMemoryOpsActions;
exports.summarizeMemoryOpsHealth = summarizeMemoryOpsHealth;
...
function stableId(prefix, ...parts) {
    ...
    const hash = node_crypto_1.default.createHash("sha256").update(parts.join("\n")).digest("hex").slice(0, 12);
    return `${prefix}:${readable}:${hash}`;
}
function maxHealth(left, right) {
    const rank = { healthy: 0, unknown: 1, degraded: 2, critical: 3 };
...
function classifyPostgresDbaFindings(snapshot) {
    if (!snapshot)
        return [];
    if (!snapshot.ok) {
        return [
    ...
    const staleAnalyzeTables = snapshot.tableStats.filter((table) => {
        if (table.liveRows < 10_000)
            return false;
        if (!table.lastAnalyze)
            return true;
...
function classifyMemoryFindings(memory) {
    const findings = [];
    const stuckItems = [];
...
function classifyServiceFindings(services) {
    return services
...
function related(findings, code) {
    return findings.filter((entry) => entry.code === code || entry.code.includes(code));
}
function deriveMemoryOpsActions(input) {
    const actions = [];
    const add = (candidate) => {
        if (!candidate || actions.some((entry) => entry.id === candidate.id))
            return;
    ...
    const connectionPressure = input.findings.filter((entry) => entry.code === "connection_utilization_critical" ||
    ...
    const duplicateProcessServices = input.services.filter((service) => service.kind === "process" && (service.health === "degraded" || service.health === "critical"));
