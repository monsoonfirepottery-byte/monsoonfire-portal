...
function renderMemberCard(row, options = {}) {
    const actions = [
        options.canViewActivity
            ? `<button type="button" class="ops-button ops-button-secondary" data-member-activity="${esc(row.uid)}">Activity</button>`
            : "",
        options.canEditProfile
            ? `<button type="button" class="ops-button ops-button-secondary" data-member-profile="${esc(row.uid)}">Profile</button>`
            : "",
        options.canEditMembership
            ? `<button type="button" class="ops-button ops-button-secondary" data-member-membership="${esc(row.uid)}">Membership</button>`
            : "",
        options.canEditRole
            ? `<button type="button" class="ops-button ops-button-secondary" data-member-role="${esc(row.uid)}">Roles</button>`
            : "",
        options.canEditBilling
            ? `<button type="button" class="ops-button ops-button-secondary" data-member-billing="${esc(row.uid)}">Billing</button>`
            : "",
    ].filter(Boolean);
    const billingSummary = row.billing?.paymentMethodSummary
        ? row.billing.paymentMethodSummary
        : row.billing?.stripeCustomerId
            ? "Stripe customer linked, no card summary on file."
            : "No billing profile attached yet.";
    return `
    <article class="ops-case ops-tone-neutral">
      ...
      <dl class="ops-meta-grid">
        ...
        <div class="ops-span-2"><dt>Billing</dt><dd>${esc(billingSummary)}</dd></div>
...
function renderMemberWorkbench(options) {
    return `
    <section class="ops-member-workbench">
      <div class="ops-panel ops-panel-member-workbench">
        <div class="ops-panel__head">
          <div>
            <p class="ops-kicker">Member workbench</p>
            <h2 id="ops-member-workbench-title">Select a member to edit</h2>
          </div>
          <div class="ops-ribbon-stack">
            <div class="ops-freshness-ribbon ops-freshness-ribbon-info">No raw card data enters Studio Brain.</div>
          </div>
        </div>
        <p class="ops-summary" id="ops-member-workbench-summary">Choose a member from the left rail. The workbench keeps edits, roles, billing-safe metadata, and activity in one place without falling back to browser prompts.</p>
        <div class="ops-stack">
          ${options.canCreateMember ? `
            <form class="ops-compose-form" id="ops-member-create-form">
              <p class="ops-kicker">Create member</p>
              <input name="displayName" placeholder="Display name" />
              <input name="email" type="email" placeholder="Email" />
              <input name="membershipTier" placeholder="Membership tier (optional)" />
              <select name="portalRole">
                <option value="member">Member</option>
                <option value="staff">Staff</option>
                <option value="admin">Admin</option>
              </select>
              <input name="opsRoles" placeholder="Ops roles (comma-separated, optional)" />
              <textarea name="staffNotes" rows="2" placeholder="Starting notes or handoff context (optional)"></textarea>
              <div class="ops-actions">
                <button type="submit" class="ops-button">Create member</button>
              </div>
            </form>
          ` : ""}
          ${(options.canEditProfile || options.canEditMembership || options.canEditRole || options.canEditBilling || options.canViewActivity) ? `
            <div class="ops-member-workbench__body">
              <div class="ops-case ops-tone-neutral">
                <p class="ops-kicker">Selected member</p>
                <h3 id="ops-member-selected-name">No member selected</h3>
                <p class="ops-summary" id="ops-member-selected-email">Choose a member card to load the workbench.</p>
                <dl class="ops-meta-grid">
                  <div><dt>UID</dt><dd id="ops-member-selected-uid">-</dd></div>
                  <div><dt>Membership</dt><dd id="ops-member-selected-membership">-</dd></div>
                  <div class="ops-span-2"><dt>Billing posture</dt><dd id="ops-member-selected-billing">Stripe-hosted only. Save tokenized references, never PAN/CVC.</dd></div>
                </dl>
              </div>
              ${options.canEditProfile ? `
                <form class="ops-compose-form" id="ops-member-profile-form">
                  <p class="ops-kicker">Profile editor</p>
                  <input type="hidden" name="uid" />
                  <input name="displayName" placeholder="Display name" />
                  <input name="kilnPreferences" placeholder="Kiln preferences" />
                  <textarea name="staffNotes" rows="3" placeholder="Staff notes"></textarea>
                  <div class="ops-actions">
                    <button type="submit" class="ops-button">Save profile</button>
                  </div>
                </form>
              ` : ""}
              ${options.canEditMembership ? `
                <form class="ops-compose-form" id="ops-member-membership-form">
                  ...
                  <input type="hidden" name="uid" />
                  <input name="membershipTier" placeholder="Membership tier" />
                  <div class="ops-actions">
                    <button type="submit" class="ops-button">Save membership</button>
                  </div>
                </form>
              ` : ""}
              ${options.canEditRole ? `
                <form class="ops-compose-form" id="ops-member-role-form">
                  <p class="ops-kicker">Access mask</p>
                  <input type="hidden" name="uid" />
                  <select name="portalRole">
                    <option value="member">Member</option>
                    <option value="staff">Staff</option>
                  ...
                  <input name="opsRoles" placeholder="owner, member_ops, support_ops, kiln_lead..." />
                  <div class="ops-actions">
                    <button type="submit" class="ops-button">Save access</button>
                  </div>
                </form>
              ` : ""}
              ${options.canEditBilling ? `
                <form class="ops-compose-form" id="ops-member-billing-form">
                  ...
                  <input type="hidden" name="uid" />
                  <input name="stripeCustomerId" placeholder="Stripe customer ID (cus_...)" />
                  <input name="defaultPaymentMethodId" placeholder="Default payment method ID (pm_...)" />
                  <div class="ops-countdown-grid ops-countdown-grid-dual">
                    <input name="cardBrand" placeholder="Card brand" />
                    <input name="cardLast4" placeholder="Last 4 only" maxlength="4" />
                    <input name="expMonth" placeholder="Exp month" maxlength="2" />
                    <input name="expYear" placeholder="Exp year" maxlength="4" />
                  </div>
                  <input name="billingContactName" placeholder="Billing contact name" />
                  <input name="billingContactEmail" type="email" placeholder="Billing contact email" />
                  <input name="billingContactPhone" placeholder="Billing contact phone" />
                  <p class="ops-summary">Card entry stays outside Studio Brain. Staff should use Stripe-hosted capture, then record only the customer ID, payment method ID, and safe summary fields here.</p>
                  <div class="ops-actions">
                    <button type="submit" class="ops-button">Save billing profile</button>
                  </div>
                </form>
              ` : ""}
              ${options.canViewActivity ? `
                <article class="ops-case ops-tone-neutral" id="ops-member-activity-panel">
                  <p class="ops-kicker">Activity</p>
                  <h3>Waiting for a member selection</h3>
                  <p class="ops-summary">Reservations, events, support load, and lending history will appear here.</p>
                </article>
              ` : ""}
            </div>
          ` : ""}
      ...
      <div class="ops-manager-canvas">
        ...
        ${renderModePanel("internet", "member-ops", `
          <div class="ops-layout">
            ...
            <div class="ops-panel">
              <div class="ops-panel__head"><div><p class="ops-kicker">Member workbench</p><h2>Edit, create, and attach safe billing metadata</h2></div></div>
              <div class="ops-scroll-stack">
        ...
        canViewActivity: canViewMembers,
                ...
                <article class="ops-case ops-tone-neutral">
                  <p class="ops-kicker">Reservation bundles</p>
                  <h3>The member promises that need prep context</h3>
...
<html>
  <head>
    ...
    <style>
      :root {
        ...
        --panel-strong: rgba(14,21,31,0.94);
        --panel-soft: rgba(23,31,42,0.82);
        --panel-muted: rgba(24,32,43,0.68);
        --line: rgba(173,191,214,0.12);
        --line-strong: rgba(173,191,214,0.18);
        --shadow: 0 22px 54px rgba(0, 0, 0, 0.44);
        --field: rgba(7,11,17,0.92);
        --field-line: rgba(173,191,214,0.16);
        --track: rgba(122,142,168,0.18);
        --good: #58c9a1;
        --good-bg: rgba(88,201,161,0.14);
        --warn: #d8ab4a;
        --warn-bg: rgba(216,171,74,0.14);
        --danger: #dd6f63;
        --danger-bg: rgba(221,111,99,0.14);
        --neutral: #95a8bc;
        --neutral-bg: rgba(149,168,188,0.14);
        --accent: #7fa7e6;
        --accent-2: #d5a16d;
        --accent-soft: rgba(127,167,230,0.16);
        --accent-2-soft: rgba(213,161,109,0.16);
      }
      * { box-sizing: border-box; }
      body {
        ...
        background:
          radial-gradient(circle at top left, rgba(127,167,230,0.18), transparent 30%),
          radial-gradient(circle at top right, rgba(213,161,109,0.12), transparent 24%),
          radial-gradient(circle at 50% 20%, rgba(255,255,255,0.04), transparent 36%),
          linear-gradient(180deg, #060a10 0%, #0c131d 42%, #111b28 100%);
      }
      ...
      .ops-kpi { padding: 16px; border-radius: 18px; border: 1px solid var(--line); background: var(--panel-soft); }
      ...
      .ops-button-secondary { background: var(--accent-2-soft); color: var(--accent-2); border-color: rgba(213,161,109,0.2); }
      .ops-chip { background: var(--panel-soft); color: var(--muted); border-color: var(--line); }
      ...
      .ops-mode-tab.is-active {
        ...
        color: #f8f3ed;
        border-color: rgba(127,167,230,0.28);
      }
      ...
      .ops-ticker-shell {
        ...
        border-radius: 18px;
        border: 1px solid var(--line);
        background: linear-gradient(90deg, rgba(13,20,29,0.96), rgba(26,35,47,0.88));
      }
      ...
      .ops-ticker-shell::before {
        left: 0;
        background: linear-gradient(90deg, rgba(9,16,25,1), rgba(9,16,25,0));
      }
      .ops-ticker-shell::after {
        right: 0;
        background: linear-gradient(270deg, rgba(9,16,25,1), rgba(9,16,25,0));
      }
      .ops-ticker-track {
        display: flex;
        gap: 12px;
      ...
      .ops-gauge__ring {
        ...
        border-radius: 50%;
        background:
          radial-gradient(circle at center, rgba(14,21,31,0.96) 0 50%, transparent 51%),
          conic-gradient(from -120deg, var(--gauge-color) 0deg var(--gauge-progress), var(--track) var(--gauge-progress) 240deg, transparent 240deg 360deg);
      ...
      .ops-gauge__needle {
        ...
        height: 46px;
        border-radius: 999px;
        background: linear-gradient(180deg, rgba(243,237,229,0.22), rgba(243,237,229,0.95));
      ...
      .ops-gauge__hub {
        ...
        border-radius: 50%;
        background: var(--ink);
        box-shadow: 0 0 0 5px rgba(7,11,17,0.82);
      }
      .ops-gauge__readout {
        position: relative;
        margin-top: 48px;
      ...
      .ops-countdown-card__ring {
        ...
        border-radius: 50%;
        background:
          radial-gradient(circle at center, rgba(14,21,31,0.98) 0 56%, transparent 57%),
          conic-gradient(from -90deg, var(--countdown-color) 0deg var(--countdown-progress), var(--track) var(--countdown-progress) 360deg);
      ...
      .ops-relay::before {
        ...
        height: 4px;
        border-radius: 999px;
        background: linear-gradient(90deg, rgba(127,167,230,0.32), rgba(213,161,109,0.28));
      }
      ...
      .ops-relay__node {
        ...
        box-shadow: 0 0 0 8px rgba(7,11,17,0.62);
      }
      .ops-relay__label {
        font-size: 0.72rem;
        text-transform: uppercase;
      ...
      .ops-map-zone__status {
        ...
        height: 14px;
        border-radius: 999px;
        box-shadow: 0 0 0 6px rgba(7,11,17,0.86);
      ...
      .ops-pill {
        ...
        color: var(--neutral);
        border: 1px solid rgba(149,168,188,0.18);
        font-size: 0.82rem;
        font-weight: 700;
      }
      ...
      .ops-chat-message-user { margin-left: auto; background: var(--accent-soft); }
      .ops-chat-message-assistant { margin-right: auto; background: var(--panel-soft); }
      .ops-chat-form, .ops-compose-form { display: grid; gap: 10px; margin-top: 14px; }
      .ops-member-workbench__body { display: grid; gap: 12px; }
      .ops-panel-member-workbench { background: linear-gradient(180deg, rgba(19,23,32,0.96), rgba(15,18,26,0.92)); }
  ...
  <body>
    ...
    <script>
      ...
      const memberWorkbenchTitle = document.getElementById("ops-member-workbench-title");
      const memberWorkbenchSummary = document.getElementById("ops-member-workbench-summary");
      const selectedMemberName = document.getElementById("ops-member-selected-name");
      const selectedMemberEmail = document.getElementById("ops-member-selected-email");
      const selectedMemberUid = document.getElementById("ops-member-selected-uid");
      const selectedMemberMembership = document.getElementById("ops-member-selected-membership");
      const selectedMemberBilling = document.getElementById("ops-member-selected-billing");
      const memberActivityPanel = document.getElementById("ops-member-activity-panel");
      const memberCreateForm = document.getElementById("ops-member-create-form");
      const memberProfileForm = document.getElementById("ops-member-profile-form");
      const memberMembershipForm = document.getElementById("ops-member-membership-form");
      const memberRoleForm = document.getElementById("ops-member-role-form");
      const memberBillingForm = document.getElementById("ops-member-billing-form");
      ...
      function updateMemberWorkbench(member) {
        currentMemberUid = member && member.uid ? member.uid : null;
        if (memberWorkbenchTitle) {
          memberWorkbenchTitle.textContent = member && member.displayName ? "Editing " + member.displayName : "Select a member to edit";
        }
        if (memberWorkbenchSummary) {
          memberWorkbenchSummary.textContent = member && member.email
            ? member.email + " · " + (member.opsRoles && member.opsRoles.length ? member.opsRoles.join(", ") : "no ops roles")
            : "Choose a member from the left rail. The workbench keeps edits, roles, billing-safe metadata, and activity in one place without falling back to browser prompts.";
        }
        if (selectedMemberName) selectedMemberName.textContent = member && member.displayName ? member.displayName : "No member selected";
        if (selectedMemberEmail) selectedMemberEmail.textContent = member && member.email ? member.email : "Choose a member card to load the workbench.";
        if (selectedMemberUid) selectedMemberUid.textContent = member && member.uid ? member.uid : "-";
        if (selectedMemberMembership) selectedMemberMembership.textContent = member && member.membershipTier ? member.membershipTier : "none";
        if (selectedMemberBilling) {
          selectedMemberBilling.textContent = member && member.billing
            ? (member.billing.paymentMethodSummary || member.billing.stripeCustomerId || "Safe billing profile only.")
            : "Stripe-hosted only. Save tokenized references, never PAN/CVC.";
        }
        setFormValue(memberProfileForm, "uid", member && member.uid);
        setFormValue(memberProfileForm, "displayName", member && member.displayName);
        setFormValue(memberProfileForm, "kilnPreferences", member && member.kilnPreferences);
        setFormValue(memberProfileForm, "staffNotes", member && member.staffNotes);
        setFormValue(memberMembershipForm, "uid", member && member.uid);
        setFormValue(memberMembershipForm, "membershipTier", member && member.membershipTier);
        setFormValue(memberRoleForm, "uid", member && member.uid);
        setFormValue(memberRoleForm, "portalRole", member && member.portalRole);
        setFormValue(memberRoleForm, "opsRoles", member && member.opsRoles ? member.opsRoles.join(", ") : "");
        setFormValue(memberBillingForm, "uid", member && member.uid);
        setFormValue(memberBillingForm, "stripeCustomerId", member && member.billing && member.billing.stripeCustomerId);
        setFormValue(memberBillingForm, "defaultPaymentMethodId", member && member.billing && member.billing.defaultPaymentMethodId);
        setFormValue(memberBillingForm, "cardBrand", member && member.billing && member.billing.cardBrand);
        setFormValue(memberBillingForm, "cardLast4", member && member.billing && member.billing.cardLast4);
        setFormValue(memberBillingForm, "expMonth", member && member.billing && member.billing.expMonth);
        setFormValue(memberBillingForm, "expYear", member && member.billing && member.billing.expYear);
        setFormValue(memberBillingForm, "billingContactName", member && member.billing && member.billing.billingContactName);
        setFormValue(memberBillingForm, "billingContactEmail", member && member.billing && member.billing.billingContactEmail);
        setFormValue(memberBillingForm, "billingContactPhone", member && member.billing && member.billing.billingContactPhone);
      }

      async function loadMemberIntoWorkbench(uid, focus) {
        const payload = await getJson("/api/ops/members/" + encodeURIComponent(uid));
        updateMemberWorkbench(payload.member || null);
        if (focus === "activity" && memberActivityPanel) {
          const activityPayload = await getJson("/api/ops/members/" + encodeURIComponent(uid) + "/activity");
          const activity = activityPayload.activity || {};
          memberActivityPanel.innerHTML = ""
            + "<p class='ops-kicker'>Activity</p>"
            + "<h3>" + String((payload.member && payload.member.displayName) || "Member").replace(/[&<>]/g, (ch) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;' }[ch])) + "</h3>"
            + "<p class='ops-summary'>"
            + (activity.reservations || 0) + " reservations · "
            + (activity.events || 0) + " upcoming events · "
            + (activity.supportThreads || 0) + " support threads · "
            + (activity.libraryLoans || 0) + " library loans."
            + "</p>";
          showBanner("Loaded member activity into the workbench.", "success");
        } else if (focus) {
          const formByFocus = {
            profile: memberProfileForm,
            membership: memberMembershipForm,
            role: memberRoleForm,
            billing: memberBillingForm,
          };
          const targetForm = formByFocus[focus];
          if (targetForm) {
            targetForm.scrollIntoView({ behavior: "smooth", block: "nearest" });
            const firstField = targetForm.querySelector("input:not([type='hidden']), textarea, select");
            if (firstField) firstField.focus();
          }
          showBanner("Loaded " + ((payload.member && payload.member.displayName) || "member") + " into the workbench.", "success");
        }
      }

      document.querySelectorAll("[data-member-activity], [data-member-profile], [data-member-membership], [data-member-role], [data-member-billing]").forEach((button) => {
        button.addEventListener("click", async () => {
          const uid =
            button.getAttribute("data-member-activity")
            || button.getAttribute("data-member-profile")
            || button.getAttribute("data-member-membership")
            || button.getAttribute("data-member-role")
            || button.getAttribute("data-member-billing");
          const focus = button.hasAttribute("data-member-activity")
            ? "activity"
            : button.hasAttribute("data-member-membership")
              ? "membership"
              : button.hasAttribute("data-member-role")
                ? "role"
                : button.hasAttribute("data-member-billing")
                  ? "billing"
                  : "profile";
          if (!uid) return;
          try {
            await loadMemberIntoWorkbench(uid, focus);
          } catch (error) {
            showBanner(error instanceof Error ? error.message : String(error), "error");
          }
        });
        ...
        memberCreateForm.addEventListener("submit", async (event) => {
          ...
          try {
            const payload = await postJson("/api/ops/members", {
              displayName: memberCreateForm.querySelector("input[name='displayName']").value.trim(),
              email: memberCreateForm.querySelector("input[name='email']").value.trim(),
              membershipTier: memberCreateForm.querySelector("input[name='membershipTier']").value.trim(),
              portalRole: memberCreateForm.querySelector("select[name='portalRole']").value,
              opsRoles: parseRoleList(memberCreateForm.querySelector("input[name='opsRoles']").value),
              staffNotes: memberCreateForm.querySelector("textarea[name='staffNotes']").value.trim(),
              reason: "Created from the autonomous ops portal.",
            });
            updateMemberWorkbench(payload.member || null);
            memberCreateForm.reset();
            showBanner("Member created. Refresh the member lane when you want the new card in the roster.", "success");
          } catch (error) {
            showBanner(error instanceof Error ? error.message : String(error), "error");
          }
        });
      ...
      if (memberProfileForm) {
        memberProfileForm.addEventListener("submit", async (event) => {
          event.preventDefault();
          const uid = memberProfileForm.querySelector("input[name='uid']").value.trim();
          if (!uid) {
            showBanner("Load a member into the workbench first.", "error");
            return;
          }
          try {
            const payload = await postJson("/api/ops/members/" + encodeURIComponent(uid) + "/profile", {
              reason: "Edited from the autonomous ops portal.",
              patch: {
                displayName: memberProfileForm.querySelector("input[name='displayName']").value.trim(),
                kilnPreferences: memberProfileForm.querySelector("input[name='kilnPreferences']").value.trim(),
                staffNotes: memberProfileForm.querySelector("textarea[name='staffNotes']").value.trim(),
              },
            });
            updateMemberWorkbench(payload.member || null);
      ...
      if (memberMembershipForm) {
        memberMembershipForm.addEventListener("submit", async (event) => {
          event.preventDefault();
          const uid = memberMembershipForm.querySelector("input[name='uid']").value.trim();
          if (!uid) {
            showBanner("Load a member into the workbench first.", "error");
            return;
          }
          try {
            const payload = await postJson("/api/ops/members/" + encodeURIComponent(uid) + "/membership", {
              membershipTier: memberMembershipForm.querySelector("input[name='membershipTier']").value.trim(),
      ...
      if (memberRoleForm) {
        memberRoleForm.addEventListener("submit", async (event) => {
          event.preventDefault();
          const uid = memberRoleForm.querySelector("input[name='uid']").value.trim();
          if (!uid) {
            showBanner("Load a member into the workbench first.", "error");
            return;
          }
          try {
            const payload = await postJson("/api/ops/members/" + encodeURIComponent(uid) + "/role", {
              portalRole: memberRoleForm.querySelector("select[name='portalRole']").value,
              opsRoles: parseRoleList(memberRoleForm.querySelector("input[name='opsRoles']").value),
              reason: "Updated from the autonomous ops portal.",
            });
            updateMemberWorkbench(payload.member || null);
            showBanner("Role assignment updated.", "success");
          } catch (error) {
            showBanner(error instanceof Error ? error.message : String(error), "error");
          }
        });
      ...
      if (memberBillingForm) {
        memberBillingForm.addEventListener("submit", async (event) => {
          event.preventDefault();
          const uid = memberBillingForm.querySelector("input[name='uid']").value.trim();
          if (!uid) {
            showBanner("Load a member into the workbench first.", "error");
            return;
          }
          try {
            const payload = await postJson("/api/ops/members/" + encodeURIComponent(uid) + "/billing", {
              reason: "Saved billing-safe metadata from the autonomous ops portal.",
              billing: {
                stripeCustomerId: memberBillingForm.querySelector("input[name='stripeCustomerId']").value.trim(),
                defaultPaymentMethodId: memberBillingForm.querySelector("input[name='defaultPaymentMethodId']").value.trim(),
                cardBrand: memberBillingForm.querySelector("input[name='cardBrand']").value.trim(),
                cardLast4: memberBillingForm.querySelector("input[name='cardLast4']").value.trim(),
                expMonth: memberBillingForm.querySelector("input[name='expMonth']").value.trim(),
                expYear: memberBillingForm.querySelector("input[name='expYear']").value.trim(),
                billingContactName: memberBillingForm.querySelector("input[name='billingContactName']").value.trim(),
                billingContactEmail: memberBillingForm.querySelector("input[name='billingContactEmail']").value.trim(),
                billingContactPhone: memberBillingForm.querySelector("input[name='billingContactPhone']").value.trim(),
              },
            });
            updateMemberWorkbench(payload.member || null);
            showBanner("Billing-safe profile updated. Raw card data still belongs only in Stripe-hosted flows.", "success");
          } catch (error) {
            showBanner(error instanceof Error ? error.message : String(error), "error");
          }
        });
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .ops-choice-card span {
        ...
        border-radius: 999px;
        font-weight: 700;
        background: rgba(127,167,230,0.16);
        color: var(--accent);
      }
      .ops-choice-card-legacy span { background: rgba(213,161,109,0.16); color: var(--accent-2); }
      .ops-choice-card-disabled { opacity: 0.72; }
      .ops-choice-notes {
        padding: 18px 22px;
        border-radius: 22px;
        border: 1px solid var(--line);
        background: rgba(22,31,42,0.78);
      }
