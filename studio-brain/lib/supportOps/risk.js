...
exports.assessSupportRisk = assessSupportRisk;
...
async function assessSupportRisk(message, policy) {
    const senderEmail = message.senderEmail?.trim().toLowerCase() ?? null;
    let senderVerifiedUid = null;
    if (senderEmail) {
        try {
            const user = await (0, auth_1.getAuth)().getUserByEmail(senderEmail);
    ...
    const blockedActionRequested = policy.blockedActions.some((action) => action && text.includes(action.toLowerCase())) ||
    ...
    const reasons = [];
    if (!senderVerifiedUid)
        reasons.push("sender_unverified");
    if (forwarded)
        reasons.push("forwarded_or_third_party_context");
    if (blockedActionRequested)
        reasons.push("blocked_action_requested");
    if (accessSecretRequested)
        reasons.push("access_secret_requested");
    if (manualOverrideLanguage)
        reasons.push("manual_override_language");
    if (suspiciousLinks.length > 0)
        reasons.push("suspicious_links");
    if (suspiciousAttachments.length > 0)
        reasons.push("suspicious_attachments");
    let state = "clear";
    if (accessSecretRequested ||
