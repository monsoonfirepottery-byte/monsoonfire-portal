...
exports.SupportOpsService = void 0;
exports.buildSupportConversationKey = buildSupportConversationKey;
exports.supportChannelName = supportChannelName;
exports.buildEmberRunId = buildEmberRunId;
exports.buildEmberMemoryScope = buildEmberMemoryScope;
exports.buildEmberMemberSubject = buildEmberMemberSubject;
exports.buildEmberPatternSubject = buildEmberPatternSubject;
exports.determineProposalCapabilityId = determineProposalCapabilityId;
exports.decideSupportAction = decideSupportAction;
...
function supportSignatureBlock() {
    return (0, persona_1.supportAgentSignatureLines)();
}
function safeStatusLine(policySlug) {
    switch (policySlug) {
...
function combinedMessageText(message) {
...
function normalizeSubjectFingerprint(subject) {
    return subject
        .toLowerCase()
...
function conversationTimeBucket(receivedAt) {
    const receivedAtMs = Date.parse(receivedAt);
    if (!Number.isFinite(receivedAtMs))
        return "unknown";
    return String(Math.floor(receivedAtMs / SUPPORT_CONVERSATION_WINDOW_MS));
}
function buildSupportConversationKey(message) {
    ...
    const anchor = references[0]
        ...
        ?? null;
    if (anchor) {
        return `support-conversation:${(0, hash_1.stableHashDeep)({
            senderEmail,
            anchor,
        }).slice(0, 32)}`;
    }
...
function supportChannelName(provider) {
    return provider === "discord" ? "discord" : "email";
}
function buildEmberRunId(channel, conversationKey) {
    return `ember-support:${channel}:${truncateText(conversationKey, 240)}`;
}
function buildEmberMemoryScope(channel, conversationKey) {
    return `run:${buildEmberRunId(channel, conversationKey)}`;
}
function buildEmberMemberSubject(value) {
    const normalized = truncateText(String(value ?? "").trim().toLowerCase(), 320);
    const subject = normalized || "unknown";
    return `ember:member:${(0, hash_1.stableHashDeep)(subject).slice(0, 24)}`;
}
function buildEmberPatternSubject(issueType) {
    const normalized = truncateText(issueType.trim().toLowerCase().replace(/[^a-z0-9:_-]+/g, "-"), 120) || "general";
    return `ember:pattern:${normalized}`;
}
function extendThreadHistory(existingSnapshot, threadId) {
    const history = new Set(existingSnapshot?.sourceThreadIds ?? []);
    if (existingSnapshot?.sourceThreadId)
        history.add(existingSnapshot.sourceThreadId);
    if (threadId)
        history.add(threadId);
    return [...history];
}
function hasFreshCareTouch(existingSnapshot, receivedAt) {
    const lastCareTouchAt = existingSnapshot?.lastCareTouchAt ?? null;
    if (!lastCareTouchAt)
        return false;
    const lastTouchMs = Date.parse(lastCareTouchAt);
    const receivedAtMs = Date.parse(receivedAt);
    if (!Number.isFinite(lastTouchMs) || !Number.isFinite(receivedAtMs))
        return false;
    return receivedAtMs - lastTouchMs <= SUPPORT_FRESH_TOUCH_WINDOW_MS;
}
function detectMemberCareReason(message, policy, existingSnapshot) {
    const text = combinedMessageText(message);
    if ((policy.policySlug === "firing-scheduling" || policy.policySlug === "storage-abandoned-work" || PICKUP_PATTERN.test(text))
...
function detectConfusionState(message, existingSnapshot) {
    const text = combinedMessageText(message);
...
function isCourtesySafe(input) {
    return Boolean(input.memberCareReason
...
function courtesyAcknowledgement(reason, playbook) {
    switch (reason) {
...
function planMemberCare(input) {
    ...
    const repeatedFollowUpWithoutFreshTouch = Boolean(input.existingSnapshot)
        && !freshTouchExists
    ...
    const baseState = !memberCareReason
        ? "none"
        : courtesySafe
...
function determineProposalCapabilityId(message, policy) {
    const text = combinedMessageText(message);
...
function decideSupportAction(input) {
...
function buildProposalPlan(input) {
    ...
    return {
        ...
        rationale: truncateText(`Support email ${input.message.messageId} for case ${input.supportRequestId} requires approval-gated handling under ${policyName}.`, 400),
        previewSummary: truncateText(`Support approval needed for ${policyName}: ${input.message.subject}`, 200),
        expectedEffects: [
...
function computeBackoffUntil(nowMs, baseMs, maxMs, failures) {
...
class SupportOpsService {
    ...
    constructor(options) {
        ...
        this.policyResolver = options.policyResolver ?? policyResolver_1.resolveSupportPolicy;
        this.riskAssessor = options.riskAssessor ?? risk_1.assessSupportRisk;
        ...
        if (!options.ingestSupportRequest) {
            ...
            if (!this.ingestBearerToken && !this.ingestBearerTokenProvider) {
                throw new Error("SupportOpsService requires ingestBearerToken or ingestBearerTokenProvider when no ingestSupportRequest override is provided.");
            }
        }
    }
    async resolveIngestBearerToken() {
        if (this.ingestBearerToken)
            return this.ingestBearerToken;
        ...
        try {
            ...
            for (const message of result.messages) {
                ...
                try {
                    ...
                    processed += 1;
                    if (outcome.replySent)
                        repliesSent += 1;
                    if (outcome.replyDrafted)
                        replyDrafts += 1;
    ...
    async processMessage(message) {
        ...
        const initialIngest = await this.ingestSupportRequest(this.buildIngestPayload({
            ...
            automationState: disposition.decision === "proposal_required" ? "pending" : disposition.automationState,
        ...
        const newThreadDriftDetected = threadDriftFlag
        ...
        const alreadyAutoHandled = existingSnapshot?.latestSourceMessageId === message.messageId
        ...
        const replyRequiresFollowUp = replyPlan.shouldSend
            && !initialIngest.replayed
            && !alreadyAutoHandled;
        const shouldSendReply = replyRequiresFollowUp
        ...
        const existingCareTouchCount = existingSnapshot && typeof existingSnapshot.careTouchCount === "number"
            ? Math.max(0, Math.trunc(existingSnapshot.careTouchCount))
            : 0;
        const careTouchCreated = shouldSendReply || replyDraftedForReview;
        const finalMemberCareState = !replyPlan.memberCareReason
            ...
            : shouldSendReply
                ...
                : replyDraftedForReview
                    ? "drafted"
                    : memberCare.memberCareState;
        const finalLastCareTouchAt = careTouchCreated
            ? nowIso
            : existingSnapshot?.lastCareTouchAt ?? null;
        const finalCareTouchCount = careTouchCreated
            ? existingCareTouchCount + 1
            : existingCareTouchCount;
        const finalAutomationState = effectiveDisposition.decision === "proposal_required"
            ? proposalId ? "proposal_created" : "staff_review"
            : replyDraftedForReview
                ? "staff_review"
                : effectiveDisposition.automationState;
        const finalQueueBucket = effectiveDisposition.decision === "proposal_required"
        ...
        const finalLastOperatorActionAt = proposalCreated || careTouchCreated || finalQueueBucket === "security_hold"
            ? nowIso
            : existingSnapshot?.lastOperatorActionAt ?? null;
        const existingMessageCount = existingSnapshot && typeof existingSnapshot.rawSnapshot?.messageCount === "number"
            ? Math.max(1, Math.trunc(existingSnapshot.rawSnapshot.messageCount))
            : 0;
        const messageCount = initialIngest.replayed
            ? Math.max(1, existingMessageCount || 1)
            : Math.max(1, existingMessageCount + 1);
        const emberWorking = this.emberMemory
    ...
    async createProposal(input) {
        const requestInput = {
            ...
            idempotencyKey: (0, hash_1.stableHashDeep)({
                supportRequestId: input.supportRequestId,
                sourceThreadId: input.message.threadId,
                sourceMessageId: input.message.messageId,
    ...
    async defaultIngestSupportRequest(payload) {
    ...
    async emitLoopSignal(input) {
        if (!this.recordLoopSignal)
            return;
        try {
    ...
    async appendAudit(action, rationale, metadata) {
        await this.options.eventStore.append({
            ...
            inputHash: (0, hash_1.stableHashDeep)(metadata),
            outputHash: null,
            metadata,
        });
    }
}
exports.SupportOpsService = SupportOpsService;
