...
exports.PostgresSupportOpsStore = void 0;
...
function hydrateCaseSnapshot(row) {
    ...
    const conversationKey = toNullableString(rawSnapshot.conversationKey)
    ...
    return {
        ...
        decision: String(row.decision ?? rawSnapshot.decision ?? "staff_review"),
        riskState: String(row.risk_state ?? rawSnapshot.riskState ?? "possible_security_risk"),
        riskReasons: toStringArray(rawSnapshot.riskReasons),
        automationState: String(row.automation_state ?? rawSnapshot.automationState ?? "staff_review"),
        queueBucket: String(row.queue_bucket ?? rawSnapshot.queueBucket ?? "staff_review"),
        unread: toBoolean(row.unread ?? rawSnapshot.unread, true),
        memberCareState: String(row.member_care_state ?? rawSnapshot.memberCareState ?? "none"),
        memberCareReason: toNullableString(row.member_care_reason ?? rawSnapshot.memberCareReason),
        ...
        lastOperatorActionAt: toNullableString(row.last_operator_action_at ?? rawSnapshot.lastOperatorActionAt),
        nextRecommendedAction: toNullableString(row.next_recommended_action ?? rawSnapshot.nextRecommendedAction),
        ...
        confusionState: String(row.confusion_state ?? rawSnapshot.confusionState ?? "none"),
        ...
        policyResolution: (rawSnapshot.policyResolution && typeof rawSnapshot.policyResolution === "object"
        ...
        proposalCapabilityId: toNullableString(row.proposal_capability_id ?? rawSnapshot.proposalCapabilityId),
...
class PostgresSupportOpsStore {
    async getMailboxState(provider, mailbox) {
        const pool = (0, postgres_1.getPgPool)();
        const result = await pool.query(`SELECT provider, mailbox, history_cursor, last_sync_at, last_success_at, consecutive_failures, backoff_until, last_error, metadata
       FROM brain_support_mailbox_state
       WHERE provider = $1 AND mailbox = $2`, [provider, mailbox]);
        if (!result.rowCount)
            return null;
    ...
    async saveMailboxState(state) {
        const pool = (0, postgres_1.getPgPool)();
        await pool.query(`INSERT INTO brain_support_mailbox_state
    ...
    async hasProcessedMessage(provider, messageId) {
        const pool = (0, postgres_1.getPgPool)();
        const result = await pool.query(`SELECT provider_message_id
       FROM brain_support_mailbox_messages
       WHERE provider = $1 AND provider_message_id = $2
       LIMIT 1`, [provider, messageId]);
        return Boolean(result.rowCount);
    }
    async saveMessageRecord(record) {
        const pool = (0, postgres_1.getPgPool)();
        await pool.query(`INSERT INTO brain_support_mailbox_messages
    ...
    async getCaseSnapshot(supportRequestId) {
       ...
       LIMIT 1`, [supportRequestId]);
        if (!result.rowCount)
            return null;
        return hydrateCaseSnapshot(result.rows[0]);
    }
    async saveCaseSnapshot(snapshot) {
        const pool = (0, postgres_1.getPgPool)();
        await pool.query(`INSERT INTO brain_support_cases
       (
    ...
    async listRecentCases(limit) {
       ...
       LIMIT $1`, [bounded]);
        return result.rows.map((row) => hydrateCaseSnapshot(row));
    }
    async getQueueSummary() {
        const pool = (0, postgres_1.getPgPool)();
        const result = await pool.query(`SELECT queue_bucket, unread, last_received_at, thread_drift_flag, member_care_state, last_care_touch_at, raw_snapshot
       FROM brain_support_cases
       WHERE queue_bucket <> 'resolved'`);
        ...
        for (const row of result.rows) {
            ...
            if (unreadFlag)
                unread += 1;
            if (bucket === "awaiting_info")
                awaitingInfo += 1;
            if (bucket === "awaiting_approval")
                awaitingApproval += 1;
            if (bucket === "security_hold")
                securityHold += 1;
            if (bucket === "staff_review" || bucket === "unread")
                staffReview += 1;
            if (row.thread_drift_flag === true)
                splitThreadSuspects += 1;
            if (lastReceivedAt) {
                if (!oldestOpenAt || lastReceivedAt < oldestOpenAt)
                    oldestOpenAt = lastReceivedAt;
                ...
                if (ageHours >= 24)
                    ...
                    fresh += 1;
                const careTouchedAfterLatestInbound = Boolean(lastCareTouchAt) && Date.parse(String(lastCareTouchAt)) >= Date.parse(lastReceivedAt);
                const repeatedFollowUpWithoutFreshTouch = messageCount >= 2 && !careTouchedAfterLatestInbound;
                const hitsWarningBandWithoutCare = ageHours >= 4 && memberCareState !== "sent" && memberCareState !== "drafted";
    ...
    async addDeadLetter(input) {
        ...
        await pool.query(`INSERT INTO brain_support_dead_letters
       (id, provider, mailbox, provider_message_id, error_message, raw_payload, attempt_count, created_at)
       VALUES ($1,$2,$3,$4,$5,$6::jsonb,$7,$8::timestamptz)`, [id, input.provider, input.mailbox, input.messageId, input.errorMessage, JSON.stringify(input.rawPayload), attemptCount, createdAt]);
        return {
            id,
    ...
    async listDeadLetters(limit) {
        const pool = (0, postgres_1.getPgPool)();
        const bounded = Math.max(1, Math.min(limit, 200));
        const result = await pool.query(`SELECT id, provider, mailbox, provider_message_id, error_message, raw_payload, attempt_count, created_at
...
exports.PostgresSupportOpsStore = PostgresSupportOpsStore;
