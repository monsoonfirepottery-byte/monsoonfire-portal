...
exports.createPostgresWikiReadStore = createPostgresWikiReadStore;
...
function mapSourceFreshness(row) {
    return {
...
function createPostgresWikiReadStore(poolProvider = postgres_1.getPgPool) {
    return {
        async getContextPack({ tenantScope, packKey }) {
            const result = await poolProvider().query(`SELECT context_pack_id, tenant_scope, pack_key, title, status, generated_text, budget, warnings,
          ...
          LIMIT 1`, [tenantScope, packKey]);
            return result.rows[0] ? mapContextPack(result.rows[0]) : null;
        },
        async listContradictions({ tenantScope, status, limit }) {
            const result = await poolProvider().query(`SELECT contradiction_id, tenant_scope, conflict_key, severity, status, claim_a_id, claim_b_id,
          ...
          LIMIT $3`, [tenantScope, status || null, limit]);
            return result.rows.map(mapContradiction);
        },
        async listSourceFreshness({ tenantScope, status, limit }) {
            const result = await poolProvider().query(`SELECT source_id, tenant_scope, source_kind, source_path, source_uri, authority_class, content_hash,
        ...
        async search({ tenantScope, query, limit }) {
            ...
            const result = await poolProvider().query(`WITH searched_claims AS (
          ...
          LIMIT $4`, [tenantScope, query, likeQuery, limit]);
