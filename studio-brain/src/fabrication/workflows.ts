...
export type IntakeRoute = "library_reuse" | "custom_job" | "escalate";

export type IntakeDecision = {
  route: IntakeRoute;
  reason: string;
  matchedLibraryItem: PrintLibraryItem | null;
  requiredEvent: "fabrication.request" | "fabrication.plan" | "fabrication.fail";
};

export type FabricationPlanningResult =
  | {
      outcome: "planned";
      eventType: "fabrication.plan";
      route: "library_reuse" | "custom_job";
      reason: string;
      matchedLibraryItem: PrintLibraryItem | null;
      job: PrintJob;
    }
...
export type LearningCapture = {
  eventType: "fabrication.complete" | "fabrication.fail";
  reuseDecision: ReuseDecision;
  notes: string[];
  nominatedLibraryItem: PrintLibraryItem | null;
};

const urgencyWeight: Record<PrintJob["urgency"], number> = {
  ops_critical: 400,
...
export function buildLibraryIndex(library: PrintLibraryItem[]): Map<string, PrintLibraryItem> {
  const index = new Map<string, PrintLibraryItem>();
...
export function classifyFabricationRequest(request: PrintJobRequest, library: PrintLibraryItem[]): IntakeDecision {
...
function availableStockGrams(stock: ConsumableStock[], material: PrintJob["material"]): number {
  return stock
    .filter((row) => row.material === material && row.status !== "quarantined")
    .reduce((sum, row) => sum + row.remainingGrams, 0);
}

function resolveDisposition(route: "library_reuse" | "custom_job"): PrintJobDisposition {
  return route === "library_reuse" ? "library_reuse" : "custom_build";
}

function resolveReuseDecision(request: PrintJobRequest, matchedLibraryItem: PrintLibraryItem | null): ReuseDecision {
  if (matchedLibraryItem) return "keep_existing_library_item";
  return request.repeatableIntent ? "promote_to_library" : "one_off_only";
}

export function planFabricationJob(
  request: PrintJobRequest,
  printer: PrinterAsset,
  library: PrintLibraryItem[],
  stock: ConsumableStock[],
  now: Date = new Date()
): FabricationPlanningResult {
  ...
  return {
    ...
    job: {
      ...
      notes: [request.purpose, request.notes].filter((value): value is string => Boolean(value && value.trim().length > 0)),
    },
  };
}

export function sortQueueByPriority(jobs: PrintJob[]): PrintJob[] {
  return [...jobs].sort((left, right) => {
    ...
    if (leftLibraryBias !== rightLibraryBias) return rightLibraryBias - leftLibraryBias;

    return left.createdAt.localeCompare(right.createdAt);
  });
}

function openTaskExists(existingTasks: MaintenanceTask[], taskType: string): boolean {
...
export function suggestMaintenanceTasks(
  ...
  const createTask = (taskType: string, title: string, description: string, conditionSignals: FailureSignal[]): void => {
    if (openTaskExists(existingTasks, taskType)) return;
    nextTasks.push({
  ...
  if (signals.includes("first_layer_failure") || signals.includes("bed_adhesion_noise")) {
    createTask(
      ...
      "Clean and reset build plate",
      "Restore first-layer reliability before the next queue item starts.",
      ["first_layer_failure", "bed_adhesion_noise"]
    );
  }
  ...
  if (signals.includes("under_extrusion") || signals.includes("toolhead_vibration")) {
    createTask(
      "nozzle_inspection",
      "Inspect nozzle and toolhead path",
      "Check for partial clogging, wear, or mounting looseness before another repeatable tooling job.",
      ["under_extrusion", "toolhead_vibration"]
    );
  }
...
export function captureLearning(job: PrintJob, outcome: FabricationOutcomeInput): LearningCapture {
