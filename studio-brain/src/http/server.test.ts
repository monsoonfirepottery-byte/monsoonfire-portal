...
import type {
  EmberSupportAttachmentInput,
  EmberSupportAttachmentPruneResult,
  EmberSupportAttachmentRecord,
  EmberSupportAttachmentStore,
...
class MemoryEmberSupportAttachmentStore implements EmberSupportAttachmentStore {
  records = new Map<string, EmberSupportAttachmentRecord>();
  payloads = new Map<string, Buffer>();

  async countActiveForSession(sessionId: string, now = new Date()): Promise<number> {
    return Array.from(this.records.values()).filter((record) => (
      record.sessionId === sessionId && Date.parse(record.expiresAt) > now.getTime()
    )).length;
  }
  ...
  async save(input: EmberSupportAttachmentInput, now = new Date()): Promise<EmberSupportAttachmentRecord> {
    ...
    const record: EmberSupportAttachmentRecord = {
      id,
...
test("ember support attachment endpoint stores small images with bridge token metadata", async () => {
  ...
  await withServer({ emberSupportBridgeToken: "bridge-secret", supportAttachmentStore }, async (baseUrl) => {
    ...
    const payload = (await response.json()) as {
      ok: boolean;
      attachment: EmberSupportAttachmentRecord;
      controls: { maxBytes: number; maxActivePerSession: number; ttlMinutes: number };
      prune: { deletedRows: number };
    };
    assert.equal(payload.ok, true);
    assert.equal(payload.attachment.sessionId, "ember_attachment_session");
    assert.equal(payload.attachment.contentType, "image/jpeg");
    assert.equal(payload.attachment.sizeBytes, jpeg.byteLength);
    assert.equal(payload.controls.maxBytes, 512 * 1024);
    assert.equal(payload.controls.maxActivePerSession, 3);
    assert.equal(payload.controls.ttlMinutes, 60);
    assert.equal(payload.prune.deletedRows, 0);
    assert.equal(supportAttachmentStore.payloads.size, 1);
  });
});

test("ember support attachment endpoint rejects spoofed images and caps active attachments", async () => {
  const supportAttachmentStore = new MemoryEmberSupportAttachmentStore();
  const jpeg = Buffer.from([0xff, 0xd8, 0xff, 0xdb, 0x00, 0x43, 0x00, 0xff, 0xd9]);
  ...
  await withServer({ adminToken: "attachment-secret", supportAttachmentStore }, async (baseUrl) => {
    ...
    assert.equal(supportAttachmentStore.records.size, 0);
  });
});
