...
import { detectSnapshotDrift } from "../studioState/drift";
import { detectOpsRecommendations } from "../swarm/ops/anomalyDetector";
import { buildMarketingDrafts, hasRecentMarketingDraft } from "../swarm/marketing/draftPipeline";
import { detectFinanceReconciliation } from "../swarm/finance/reconciliation";

export const computeStudioStateJob: JobHandler = async (ctx) => {
  const env = readEnv();
  const current = await computeStudioState(process.env.FIREBASE_PROJECT_ID, env.STUDIO_BRAIN_SCAN_LIMIT);
  ...
  const drift = detectSnapshotDrift(latest, current, {
  ...
  if (drift.length > 0) {
    ...
    for (const row of drift.slice(0, 10)) {
      warnings.push(
        `drift:${row.metric}: expected=${row.expected} observed=${row.observed} delta=${row.delta} ratio=${row.deltaRatio.toFixed(3)}`
      );
    }
    current.diagnostics = {
  ...
  const recommendations = detectOpsRecommendations(current, previous, {
    now: new Date(),
    recentEvents,
  });
  for (const draft of recommendations.emitted) {
    await ctx.eventStore.append({
      ...
      outputHash: stableHashDeep(draft),
      metadata: draft,
    });
  }
  await ctx.eventStore.append({
    ...
    outputHash: stableHashDeep(recommendations),
    metadata: {
  ...
  const financeReconciliation = detectFinanceReconciliation(current, previous, {
    now: new Date(),
    recentEvents,
  });
  ...
  await ctx.eventStore.append({
    ...
    outputHash: stableHashDeep(financeReconciliation),
    metadata: {
  ...
  if (!hasRecentMarketingDraft(recentEvents, current.snapshotDate, 360, new Date())) {
    const drafts = buildMarketingDrafts(current);
  ...
  if (drift.length > 0) {
    await ctx.eventStore.append({
      ...
      outputHash: stableHashDeep(drift),
      metadata: {
        threshold: {
          absolute: env.STUDIO_BRAIN_DRIFT_ABSOLUTE_THRESHOLD,
          ratio: env.STUDIO_BRAIN_DRIFT_RATIO_THRESHOLD,
  ...
  await ctx.eventStore.append({
    ...
    outputHash: stableHashDeep(current),
    metadata: {
      snapshotDate: current.snapshotDate,
