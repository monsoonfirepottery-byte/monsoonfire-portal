...
export class PostgresKilnStore implements KilnStore {
  ...
  async getKiln(id: string): Promise<Kiln | null> {
    ...
    return result.rowCount ? asRecord<Kiln>(result.rows[0].raw_payload) : null;
  }

  async listKilns(): Promise<Kiln[]> {
    const pool = getPgPool();
    const result = await pool.query("SELECT raw_payload FROM brain_kilns ORDER BY display_name ASC");
    return result.rows.map((row) => asRecord<Kiln>(row.raw_payload));
  }
  ...
  async getLatestCapabilityDocument(kilnId: string): Promise<KilnCapabilityDocument | null> {
    ...
    const result = await pool.query(
      "SELECT raw_payload FROM brain_kiln_capability_documents WHERE kiln_id = $1 ORDER BY generated_at DESC LIMIT 1",
      [kilnId],
    );
    return result.rowCount ? asRecord<KilnCapabilityDocument>(result.rows[0].raw_payload) : null;
  }
  ...
  async getArtifactRecord(id: string): Promise<RawArtifactRef | null> {
    ...
    return result.rowCount ? asRecord<RawArtifactRef>(result.rows[0].raw_payload) : null;
  }

  async findArtifactBySha256(sha256: string): Promise<RawArtifactRef | null> {
    const pool = getPgPool();
    const result = await pool.query(
      "SELECT raw_payload FROM brain_kiln_artifacts WHERE sha256 = $1 ORDER BY observed_at DESC NULLS LAST LIMIT 1",
      [sha256],
    );
    return result.rowCount ? asRecord<RawArtifactRef>(result.rows[0].raw_payload) : null;
  }
  ...
  async listArtifactsForKiln(kilnId: string, limit = 20): Promise<RawArtifactRef[]> {
    const pool = getPgPool();
    const result = await pool.query(
      "SELECT raw_payload FROM brain_kiln_artifacts WHERE kiln_id = $1 ORDER BY observed_at DESC NULLS LAST LIMIT $2",
      [kilnId, Math.max(1, limit)],
    );
    return result.rows.map((row) => asRecord<RawArtifactRef>(row.raw_payload));
  }
  ...
  async getImportRun(id: string): Promise<KilnImportRun | null> {
    ...
    return result.rowCount ? asRecord<KilnImportRun>(result.rows[0].raw_payload) : null;
  }
  ...
  async getFiringRun(id: string): Promise<FiringRun | null> {
    ...
    return result.rowCount ? asRecord<FiringRun>(result.rows[0].raw_payload) : null;
  }
  ...
  async listFiringRuns(query: FiringRunQuery = {}): Promise<FiringRun[]> {
    ...
    return result.rows.map((row) => asRecord<FiringRun>(row.raw_payload));
  }
  ...
  async listFiringEvents(firingRunId: string, limit = 100): Promise<FiringEvent[]> {
    const pool = getPgPool();
    const result = await pool.query(
      "SELECT raw_payload FROM brain_kiln_firing_events WHERE firing_run_id = $1 ORDER BY ts ASC LIMIT $2",
      [firingRunId, Math.max(1, limit)],
    );
    return result.rows.map((row) => asRecord<FiringEvent>(row.raw_payload));
  }
  ...
  async listTelemetryPoints(firingRunId: string, limit = 500): Promise<TelemetryPoint[]> {
    ...
    return result.rows.map((row) => asRecord<TelemetryPoint>(row.raw_payload)).reverse();
  }
  ...
  async getLatestHealthSnapshot(kilnId: string): Promise<KilnHealthSnapshot | null> {
    const pool = getPgPool();
    const result = await pool.query(
      "SELECT raw_payload FROM brain_kiln_health_snapshots WHERE kiln_id = $1 ORDER BY ts DESC LIMIT 1",
      [kilnId],
    );
    return result.rowCount ? asRecord<KilnHealthSnapshot>(result.rows[0].raw_payload) : null;
  }
  ...
  async listOperatorActions(query: OperatorActionQuery = {}): Promise<OperatorAction[]> {
    ...
    return result.rows.map((row) => asRecord<OperatorAction>(row.raw_payload));
  }
}
