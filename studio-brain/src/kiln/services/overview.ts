...
import { inferRunPhase } from "./analytics";

export type KilnDetailView = {
  kiln: Awaited<ReturnType<KilnStore["getKiln"]>>;
  capabilityDocument: Awaited<ReturnType<KilnStore["getLatestCapabilityDocument"]>>;
  currentRun: FiringRun | null;
  recentRuns: FiringRun[];
  recentArtifacts: RawArtifactRef[];
  requiredActions: OperatorAction[];
  recentOperatorActions: OperatorAction[];
  healthSnapshot: Awaited<ReturnType<KilnStore["getLatestHealthSnapshot"]>>;
  currentRunEvents: FiringEvent[];
  currentRunTelemetry: TelemetryPoint[];
};

function latestTelemetryPoint(points: TelemetryPoint[]): TelemetryPoint | null {
  return points.length ? points[points.length - 1]! : null;
}

function connectivityState(lastSeenAt: string | null): KilnCardOverview["connectivityState"] {
  if (!lastSeenAt) return "unknown";
  const ageMs = Date.now() - Date.parse(lastSeenAt);
  if (!Number.isFinite(ageMs)) return "unknown";
  if (ageMs <= 15 * 60_000) return "online";
  return "stale";
}

function timeRunningSec(run: FiringRun | null): number | null {
...
function buildRequiredAction(run: FiringRun): OperatorAction {
  return {
    id: `required_${run.id}`,
    kilnId: run.kilnId,
    firingRunId: run.id,
    ...
    checklistJson: {},
    notes:
      run.queueState === "ready_for_start"
        ? "Operator must confirm local load and press Start on the controller."
        : "Operator follow-up required.",
  };
}

function deriveRequiredActions(currentRun: FiringRun | null, persistedActions: OperatorAction[]): OperatorAction[] {
...
export async function buildKilnOverview(
  store: KilnStore,
  input: { enableSupportedWrites: boolean; providerSupport?: KilnObservationProviderSupport | null },
): Promise<KilnOverview> {
  const kilns = await store.listKilns();
  const cards: KilnCardOverview[] = [];
  const requiredOperatorActions: OperatorAction[] = [];
  const maintenanceFlags: KilnOverview["maintenanceFlags"] = [];
  ...
  for (const kiln of kilns) {
    ...
    const nextQueuedRun = recentRuns.find(
      (entry) => entry.id !== currentRun?.id && (entry.status === "queued" || entry.status === "armed"),
    ) ?? null;
    ...
    const spread =
      latestPoint && [latestPoint.tempZone1, latestPoint.tempZone2, latestPoint.tempZone3].filter((value) => typeof value === "number").length >= 2
        ? Math.max(
            ...[latestPoint.tempZone1, latestPoint.tempZone2, latestPoint.tempZone3].filter((value): value is number => typeof value === "number"),
          )
          - Math.min(
            ...[latestPoint.tempZone1, latestPoint.tempZone2, latestPoint.tempZone3].filter((value): value is number => typeof value === "number"),
          )
    ...
    cards.push({
      ...
      inferredPhase: currentRun ? inferRunPhase(currentRun) : "Idle",
      zoneSpread: spread,
      ...
      controlPosture: deriveKilnControlPosture({
        capabilityDocument,
...
export async function buildKilnDetail(store: KilnStore, kilnId: string): Promise<KilnDetailView> {
...
export async function buildFiringRunDetail(
  store: KilnStore,
  runId: string,
): Promise<{
  run: FiringRun | null;
  events: Awaited<ReturnType<KilnStore["listFiringEvents"]>>;
  telemetry: Awaited<ReturnType<KilnStore["listTelemetryPoints"]>>;
}> {
