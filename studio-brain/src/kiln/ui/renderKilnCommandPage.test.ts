...
import { renderKilnCommandPage } from "./renderKilnCommandPage";

test("Kiln Command page is explicit about overlay posture and control badges", () => {
  ...
  assert.match(html, /without implying unsupported remote writes/i);
  assert.match(html, /Stage a firing run/i);
  assert.match(html, /Record local Start pressed/i);
  assert.match(html, /Import Genesis evidence/i);
});
