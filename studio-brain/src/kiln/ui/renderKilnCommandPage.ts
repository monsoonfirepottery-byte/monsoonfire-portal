import type { FiringQueueState } from "../domain/model";
import type { KilnCommandPageModel } from "./contracts";

const queueOptions: Array<{ value: FiringQueueState; label: string }> = [
...
function selectedQueueOption(value: FiringQueueState): string {
  return queueOptions
    .map((option) => `<option value="${esc(option.value)}"${option.value === value ? " selected" : ""}>${esc(option.label)}</option>`)
    .join("");
}

function renderTelemetryTail(model: KilnCommandPageModel["kilnDetails"][number]): string {
  ...
  return model.currentRunTelemetry
    ...
    .map(
      (point) =>
        `<li>${esc(formatTimestamp(point.ts))} · primary ${esc(formatTemp(point.tempPrimary))} · set ${esc(formatTemp(point.setPoint))} · seg ${esc(point.segment ?? "n/a")}</li>`,
    )
    .join("");
}

function renderCurrentRunEvents(model: KilnCommandPageModel["kilnDetails"][number]): string {
  ...
  return model.currentRunEvents
    ...
    .map(
      (event) =>
        `<li>${esc(formatTimestamp(event.ts))} · ${esc(event.eventType)} · ${esc(event.severity)} · ${esc(event.confidence)}</li>`,
    )
    .join("");
}

function renderRecentActions(model: KilnCommandPageModel["kilnDetails"][number]): string {
  ...
  return model.recentOperatorActions
    ...
    .map(
      (action) =>
        `<li>${esc(action.actionType)} · ${esc(formatTimestamp(action.completedAt ?? action.requestedAt))} · ${esc(action.notes ?? "Recorded without notes.")}</li>`,
    )
    .join("");
}

function renderArtifacts(model: KilnCommandPageModel["kilnDetails"][number]): string {
  ...
  return model.recentArtifacts
    ...
    .map(
      (artifact) =>
        `<li><a href="/api/kiln/artifacts/${encodeURIComponent(artifact.id)}/content">${esc(artifact.filename)}</a> · ${esc(artifact.sourceLabel ?? artifact.artifactKind)} · ${esc(formatTimestamp(artifact.observedAt))}</li>`,
    )
    .join("");
}

function renderRequiredActions(model: KilnCommandPageModel["kilnDetails"][number]): string {
  ...
  return model.requiredActions
    ...
    .map(
      (action) =>
        `<li>${esc(action.actionType)} · ${esc(action.notes ?? "Operator acknowledgement required.")}</li>`,
    )
    .join("");
}

function renderQuickActionButtons(model: KilnCommandPageModel["kilnDetails"][number]): string {
  ...
  return [
    ...
    .map(
      (action) =>
        `<button type="button" class="button"${action.runScoped ? runDisabled : ""} data-quick-action="${esc(action.actionType)}" data-kiln-id="${esc(model.kiln?.id ?? "")}" data-run-id="${esc(runId)}">${esc(action.label)}</button>`,
    )
    .join("");
}

export function renderKilnCommandPage(model: KilnCommandPageModel): string {
              ...
              ${
                detail.recentRuns.length
                  ? detail.recentRuns
                      ...
                      .map(
                        (recentRun) =>
                          `<li>${esc(recentRun.programName ?? "Program")} · ${esc(recentRun.status)} · ${esc(recentRun.queueState)} · ${esc(formatTimestamp(recentRun.startTime))}</li>`,
                      )
                      .join("")
  ...
  const requiredActions = model.overview.requiredOperatorActions
    .map(
      (action) => `<li><strong>${esc(action.actionType)}</strong> · kiln ${esc(action.kilnId)} · ${esc(action.notes ?? "Operator acknowledgement required.")}</li>`,
    )
    .join("");
  const recentFirings = model.overview.recentFirings
    .map(
      (run) => `<li>${esc(run.programName ?? "Program")} · ${esc(run.status)} · ${esc(run.queueState)} · kiln ${esc(run.kilnId)}</li>`,
    )
    .join("");
  const maintenanceFlags = model.overview.maintenanceFlags
    .map(
      (entry) => `<li>${esc(entry.kilnId)} · ${esc(entry.warnings.join(" | ") || entry.confidenceNotes.join(" | ") || "No current maintenance warnings.")}</li>`,
    )
    .join("");
