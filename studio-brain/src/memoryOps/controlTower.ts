...
import type { MemoryOpsAction, MemoryOpsHealth, MemoryOpsSnapshot } from "./contracts";
...
function healthToCardHealth(health: MemoryOpsHealth): ControlTowerServiceCard["health"] {
  if (health === "critical") return "error";
  if (health === "degraded" || health === "unknown") return "waiting";
  return "healthy";
}

function actionLabel(policy: MemoryOpsAction["policy"]): string {
  if (policy === "safe_auto") return "Auto repair";
  if (policy === "approval_required") return "Approve recovery";
  return "Open runbook";
}

export type MemoryOpsControlTowerSummary = {
  ...
  services: Array<{ id: string; label: string; health: MemoryOpsHealth; summary: string }>;
};

export function summarizeMemoryOpsForControlTower(snapshot: MemoryOpsSnapshot | null): MemoryOpsControlTowerSummary | null {
...
export function buildMemoryOpsServiceCards(snapshot: MemoryOpsSnapshot | null): ControlTowerServiceCard[] {
  if (!snapshot) {
    return [
      {
...
export function buildMemoryOpsAttention(snapshot: MemoryOpsSnapshot | null): ControlTowerAttentionItem[] {
  if (!snapshot) {
    return [{
...
export function buildMemoryOpsNextMoves(snapshot: MemoryOpsSnapshot | null): ControlTowerNextAction[] {
  if (!snapshot) {
    return [{
...
export function buildMemoryOpsEvents(snapshot: MemoryOpsSnapshot | null): ControlTowerEvent[] {
  if (!snapshot) return [];
