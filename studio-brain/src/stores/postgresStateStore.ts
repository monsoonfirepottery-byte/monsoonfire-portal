...
export class PostgresStateStore implements StateStore {
  ...
  async getLatestOverseerRun(): Promise<OverseerRunRecord | null> {
    const pool = getPgPool();
    const result = await pool.query(
      "SELECT raw_payload FROM brain_overseer_runs ORDER BY computed_at DESC LIMIT 1"
    );
    if (!result.rowCount) return null;
    return result.rows[0].raw_payload as OverseerRunRecord;
  }
  ...
  async listRecentOverseerRuns(limit: number): Promise<OverseerRunRecord[]> {
    ...
    return result.rows.map((row) => row.raw_payload as OverseerRunRecord);
  }
