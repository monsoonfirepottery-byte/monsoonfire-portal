...
const LIBRARY_SCAN_REQUEST_TIMEOUT_MS = 20_000;
const LIBRARY_IMPORT_REQUEST_TIMEOUT_MS = 45_000;
...
export default function StaffView({
  ...
  const [activeWorkshopProgrammingClusterLabel, setActiveWorkshopProgrammingClusterLabel] = useState("");
  ...
  const handleLoadWorkshopProgrammingCluster = useCallback(
    (cluster: WorkshopProgrammingCluster) => {
      const now = Date.now();
      const clusterEvents = events.filter(
        (event) => inferWorkshopProgrammingTechnique(event.title).key === cluster.key
      );
      const preferredEvent =
        [...clusterEvents]
          .filter((event) => event.status !== "cancelled")
          .sort((left, right) => {
            const leftUpcoming = left.startAtMs > now ? 1 : 0;
            const rightUpcoming = right.startAtMs > now ? 1 : 0;
            if (leftUpcoming !== rightUpcoming) return rightUpcoming - leftUpcoming;
            if (left.waitlistCount !== right.waitlistCount) return right.waitlistCount - left.waitlistCount;
      ...
      const referenceTitle = preferredEvent?.title.trim() ?? "";
      const hasDuplicateReferenceTitle = referenceTitle
        ? clusterEvents.filter((event) => event.title.trim().toLowerCase() === referenceTitle.toLowerCase()).length > 1
        : false;
      const suggestedTitle = referenceTitle
        ? cluster.waitlistCount >= 5 || cluster.openSeats <= 4
          ? `${referenceTitle} - Added Session`
          : referenceTitle
        : `${cluster.label} Workshop`;
      const suggestedCapacity = Math.max(
        8,
        Math.min(24, Math.max(Number(preferredEvent?.capacity) || 0, 12) + Math.min(cluster.waitlistCount, 8))
      );
      const searchTerm = preferredEvent
        ? hasDuplicateReferenceTitle
          ? preferredEvent.id
          : referenceTitle || preferredEvent.id
        : cluster.label.trim();

      setEventSearch(searchTerm);
      setEventStatusFilter("all");
      setActiveWorkshopProgrammingClusterLabel(cluster.label);
      if (preferredEvent?.id) {
        setSelectedEventId(preferredEvent.id);
      }
      setEventCreateDraft((prev) => ({
        ...prev,
        title: suggestedTitle,
        capacity: String(suggestedCapacity),
        location: preferredEvent?.location?.trim() || prev.location || "Monsoon Fire Studio",
      }));
      setStatus(
        preferredEvent
          ? `Loaded ${cluster.label} into quick planning and focused ${preferredEvent.title}.`
          : `Loaded ${cluster.label} into quick planning. Add date/time to schedule the next session.`
      );
      track("staff_workshops_programming_cluster_loaded", {
        cluster: cluster.key,
        label: cluster.label,
        gapScore: cluster.gapScore,
        demandScore: cluster.demandScore,
        waitlist: cluster.waitlistCount,
        hasReferenceEvent: Boolean(preferredEvent),
      });
    },
    [events]
  );
  ...
  const importLibraryIsbns = useCallback(
    ...
    ): Promise<ImportLibraryIsbnsResponse> => {
      try {
  ...
  const handleLendingIsbnImport = useCallback(async (): Promise<ImportLibraryIsbnsResponse | null> => {
    ...
    try {
      const response = await importLibraryIsbns(
        isbns,
        "csv",
        Math.max(LIBRARY_IMPORT_REQUEST_TIMEOUT_MS, Math.min(90_000, isbns.length * 6_000))
      );
  ...
  const handleLendingIsbnScanSubmit = useCallback(async (): Promise<LendingScanSubmitResult | null> => {
    ...
    try {
      const response = await importLibraryIsbns([raw], "scanner", LIBRARY_SCAN_REQUEST_TIMEOUT_MS);
      const errorCount = response.errors?.length ?? 0;
      const manualPassCount = response.manualPassRequired?.length ?? 0;
      const rejectedCount = response.rejected?.length ?? 0;
      setIsbnScanStatus(
  ...
  const hasAutoLoadedEventsSurfaceRef = useRef(false);

  useEffect(() => {
    const isEventsSurfaceActive = cockpitTab === "operations" && moduleKey === "events";
    if (!isEventsSurfaceActive) {
      hasAutoLoadedEventsSurfaceRef.current = false;
      return;
    }
    if (hasAutoLoadedEventsSurfaceRef.current) return;
    hasAutoLoadedEventsSurfaceRef.current = true;
    void loadEvents().catch(markErr);
  }, [cockpitTab, loadEvents, markErr, moduleKey]);
  ...
  const createQuickEvent = async () => {
    ...
    setActiveWorkshopProgrammingClusterLabel("");
    await loadEvents();
  ...
  const eventsContent = (
    <EventsModule
      ...
      handleLoadWorkshopProgrammingCluster={handleLoadWorkshopProgrammingCluster}
      activeWorkshopProgrammingClusterLabel={activeWorkshopProgrammingClusterLabel}
  ...
  // Cockpit-mode consolidation: render all staff operational flows through the cockpit shell for
  // consistent module entry/exit behavior and telemetry tracking.
