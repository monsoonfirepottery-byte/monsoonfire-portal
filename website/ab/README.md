...
```bash
node website/scripts/serve-ab.mjs --root website
```

Or via npm:

```bash
npm run website:serve:ab
```

Compatibility shim (legacy/optional):

```sh
node ./scripts/ps1-run.mjs website/serve-ab.ps1
```

For primary Studio Brain onboarding and CI-style flows, prefer `node website/scripts/serve-ab.mjs` or `npm run website:serve:ab`.
