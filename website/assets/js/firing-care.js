(() => {
  ...
  const themeKey = "mf:firingCareTheme";
  const a11yKey = "mf:firingCareA11y";
  ...
  const panel = document.getElementById("site-a11y-panel");
  ...
  const chatSessionKey = "mf:firingCareEmberSession";
  const chatTranscriptKey = "mf:firingCareEmberTranscript";
  const chatStateKey = "mf:firingCareEmberState";
  const chatDraftKey = "mf:firingCareEmberDraft";
  ...
  document.querySelectorAll("[data-ember-chat]").forEach((chat) => {
    ...
    const updateThreadCard = () => {
      ...
      if (transcript.length) {
        ...
        threadDetail.textContent = "This browser remembers the latest notes while you stay on this page.";
        return;
      }
      ...
      try {
        ...
        const message = error && (error.status === 429 || error.code === "RATE_LIMITED" || error.code === "EMAIL_RATE_LIMITED")
          ...
          : error && error.message && error.status
            ? error.message
            : "I can't reach the studio support link from this page. Open the studio account so staff can keep the request tied together.";
    ...
    const prepareAttachmentImage = async (file) => {
      ...
      if (file.size > 8 * 1024 * 1024) {
        throw new Error("That photo is too large for the photo uploader.");
      }
      ...
      if (!blob || blob.size > maxAttachmentBytes) {
        throw new Error("That photo still compresses too large for this page.");
      }
    ...
    const uploadAttachment = async (file) => {
      ...
      try {
        ...
        const message = error && (error.status === 429 || error.code === "ATTACHMENT_RATE_LIMITED")
          ...
          : error && error.message
            ? error.message
            : "I could not attach that photo from this page.";
        addBubble(message, "ember", true);
